...
function _QueryInterface_(self::AbstractDispatcherTrace, iid)
    ...
    if !(rc)
        _trace_(
            self,
            "in $(repr(self.policy._obj_))._QueryInterface_ with unsupported IID $(IIDToInterfaceName(iid)) ($(iid))",
        )
    end
    return rc
end
...
function _InvokeEx_(
    ...
    _trace_(
        self,
        "in $(self.policy._obj_)._InvokeEx_-$(dispid)$(args) [$(wFlags),$(lcid),$(serviceProvider)]",
    )
...
function _HandleException_(self::AbstractDispatcherWin32dbg)
    ...
    try
        ...
        if exn isa Exception
            debug = get_option(
                win32com_.gen_py.debugger.GetDebugger(),
