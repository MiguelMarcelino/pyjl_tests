...
function _InvokeEx_(self::DispatcherBase, dispid, lcid, wFlags, args, kwargs, serviceProvider)
try
...
if !(rc)
_trace_(self, "in %s._QueryInterface_ with unsupported IID %s (%s)" % (repr(self.policy._obj_), IIDToInterfaceName(iid), iid))
end
return rc
end
...
function _InvokeEx_(self::DispatcherTrace, dispid, lcid, wFlags, args, kwargs, serviceProvider)
_trace_(self, "in %r._InvokeEx_-%s%r [%x,%s,%r]" % (self.policy._obj_, dispid, args, wFlags, lcid, serviceProvider))
