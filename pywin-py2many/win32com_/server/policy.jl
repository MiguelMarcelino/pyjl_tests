...
function _CreateInstance_(self::AbstractBasicWrapPolicy, clsid, reqIID)
    ...
    try
        ...
        if exn isa win32api.error
            throw(
                error(
                    "The object is not correctly registered - $((regSpec % clsid)) key can not be read",
                ),
            )
        end
    end
...
function _invokeex_(
    self::AbstractDesignatedWrapPolicy,
    dispid,
    lcid,
    wFlags,
    args,
    ...
    if wFlags & __or__(DISPATCH_PROPERTYPUT, DISPATCH_PROPERTYPUTREF)
        ...
        if type_((
           ...
           type_((
            hasfield(typeof(self._obj_), :"Set" + name) ?
            getfield(self._obj_, :"Set" + name) : nothing
        )) == types.MethodType
...
function _transform_args_(
    self::AbstractEventHandlerPolicy,
    args,
    kwArgs,
    dispid,
    lcid,
