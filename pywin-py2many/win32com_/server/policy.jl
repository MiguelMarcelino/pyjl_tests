...
#= Create a new instance of the specified IID
    ...
    specific policy documentation for more details.
     =#
try
addnPaths = split(RegQueryValue(win32con.HKEY_CLASSES_ROOT, regAddnPath % clsid), ";")
...
if exn isa win32api.error
throw(error("The object is not correctly registered - %s key can not be read" % (regSpec % clsid)))
end
end
...
function _InvokeEx_(self::BasicWrapPolicy, dispid, lcid, wFlags, args, kwargs, serviceProvider)
...
universal_data = RegisterInterfaces(tlb_guid, tlb_lcid, tlb_major, tlb_minor, interfaces)
else
universal_data = []
end
...
function _invokeex_(self::DesignatedWrapPolicy, dispid, lcid, wFlags, args, kwArgs, serviceProvider)
...
function _transform_args_(self::EventHandlerPolicy, args, kwArgs, dispid, lcid, wFlags, serviceProvider)
ret = []
for arg in args
arg_type = type_(arg)
...
function _invokeex_(self::EventHandlerPolicy, dispid, lcid, wFlags, args, kwArgs, serviceProvider)
...
function _invokeex_(self::DynamicPolicy, dispid, lcid, wFlags, args, kwargs, serviceProvider)
try
