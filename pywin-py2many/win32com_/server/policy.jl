...
import win32con
import pythoncom
using pythoncom: DISPATCH_METHOD, DISPATCH_PROPERTYGET, DISPATCH_PROPERTYPUT, DISPATCH_PROPERTYPUTREF, DISPID_UNKNOWN, DISPID_VALUE, DISPID_PROPERTYPUT, DISPID_NEWENUM, DISPID_EVALUATE, DISPID_CONSTRUCTOR, DISPID_DESTRUCTOR, DISPID_COLLECT, DISPID_STARTENUM
S_OK = 0
...
#= Create a new instance of the specified IID
    ...
    specific policy documentation for more details.
     =#
try
addnPaths = split(win32api.RegQueryValue(win32con.HKEY_CLASSES_ROOT, regAddnPath % clsid), ";")
...
function _CreateInstance_(self::AbstractBasicWrapPolicy, clsid, reqIID)
...
function _wrap_(self::AbstractBasicWrapPolicy, object)
...
function _QueryInterface_(self::AbstractBasicWrapPolicy, iid)::Int64
...
function _query_interface_(self::AbstractBasicWrapPolicy, iid)::Int64
...
function _Invoke_(self::AbstractBasicWrapPolicy, dispid, lcid, wFlags, args)
...
function _GetIDsOfNames_(self::AbstractBasicWrapPolicy, names, lcid)
#= The main COM entry-point for GetIDsOfNames.

        This checks the validity of the arguments, and calls the _getidsofnames_ helper.
         =#
if length(names) > 1
throw(COMException(scode = winerror.DISP_E_INVALID, desc = "Cannot support member argument names"))
end
return _getidsofnames_(self, names, lcid)
end

function _getidsofnames_(self::AbstractBasicWrapPolicy, names, lcid)::Tuple
return (_getdispid_(self, names[1], 0),)
end

function _GetDispID_(self::AbstractBasicWrapPolicy, name, fdex)
return _getdispid_(self, name, fdex)
end

function _getdispid_(self::AbstractBasicWrapPolicy, name, fdex)::Dict
try
...
function _InvokeEx_(self::AbstractBasicWrapPolicy, dispid, lcid, wFlags, args, kwargs, serviceProvider)
...
function _DeleteMemberByName_(self::AbstractBasicWrapPolicy, name, fdex)
...
function _DeleteMemberByDispID_(self::AbstractBasicWrapPolicy, id)
...
function _GetMemberProperties_(self::AbstractBasicWrapPolicy, id, fdex)
...
function _GetMemberName_(self::AbstractBasicWrapPolicy, dispid)
...
function _GetNextDispID_(self::AbstractBasicWrapPolicy, fdex, dispid)::Vector
...
function _GetNameSpaceParent_(self::AbstractBasicWrapPolicy)
...
function _wrap_(self::AbstractMappedWrapPolicy, object)
...
function _getmembername_(self::AbstractMappedWrapPolicy, dispid)
...
function _wrap_(self::AbstractDesignatedWrapPolicy, ob)
tlb_guid = (hasfield(typeof(ob), :_typelib_guid_) ? 
                getfield(ob, :_typelib_guid_) : nothing)
if tlb_guid !== nothing
tlb_major, tlb_minor = (hasfield(typeof(ob), :_typelib_version_) ? 
                getfield(ob, :_typelib_version_) : (1, 0))
tlb_lcid = (hasfield(typeof(ob), :_typelib_lcid_) ? 
                getfield(ob, :_typelib_lcid_) : 0)
interfaces = [i for i in (hasfield(typeof(ob), :_com_interfaces_) ? 
                getfield(ob, :_com_interfaces_) : []) if type_(i) != pywintypes.IIDType && !startswith(i, "{") ]
...
throw(error("Object does not support DesignatedWrapPolicy, as it does not have either _public_methods_ or _typelib_guid_ attributes."))
end
...
for name in (hasfield(typeof(ob), :_public_methods_) ? 
                getfield(ob, :_public_methods_) : [])
dispid = get(self._name_to_dispid_, lower(name))
if dispid === nothing
dispid = next_dispid
self._name_to_dispid_[lower(name) + 1] = dispid
...
function _build_typeinfos_(self::AbstractDesignatedWrapPolicy)::Vector
tlb_guid = (hasfield(typeof(self._obj_), :_typelib_guid_) ? 
                getfield(self._obj_, :_typelib_guid_) : nothing)
if tlb_guid === nothing
return []
end
tlb_major, tlb_minor = (hasfield(typeof(self._obj_), :_typelib_version_) ? 
...
function _GetTypeInfoCount_(self::AbstractDesignatedWrapPolicy)::Int64
...
function _GetTypeInfo_(self::AbstractDesignatedWrapPolicy, index, lcid)
if self._typeinfos_ === nothing
self._typeinfos_ = _build_typeinfos_(self)
end
...
function _allocnextdispid(self::AbstractDesignatedWrapPolicy, last_dispid)
while true
...
function _invokeex_(self::AbstractDesignatedWrapPolicy, dispid, lcid, wFlags, args, kwArgs, serviceProvider)
...
if type_((hasfield(typeof(self._obj_), :name) ? 
                getfield(self._obj_, :name) : nothing)) == types.MethodType && type_((hasfield(typeof(self._obj_), :"Set" + name) ? 
                getfield(self._obj_, :"Set" + name) : nothing)) == types.MethodType
...
function _transform_args_(self::AbstractEventHandlerPolicy, args, kwArgs, dispid, lcid, wFlags, serviceProvider)
ret = []
for arg in args
arg_type = type_(arg)
...
try
arg = win32com_.client.Dispatch(QueryInterface(arg, pythoncom.IID_IDispatch))
catch exn
...
function _invokeex_(self::AbstractEventHandlerPolicy, dispid, lcid, wFlags, args, kwArgs, serviceProvider)
args, kwArgs = _transform_args_(self, args, kwArgs, dispid, lcid, wFlags, serviceProvider)
...
function _wrap_(self::AbstractDynamicPolicy, object)
BasicWrapPolicy._wrap_(self, object)
...
self._dyn_dispid_to_name_ = Dict(DISPID_VALUE => "_value_", DISPID_NEWENUM => "_NewEnum")
end

function _getdispid_(self::AbstractDynamicPolicy, name, fdex)::Int64
lname = lower(name)
try
...
function _invoke_(self::AbstractDynamicPolicy, dispid, lcid, wFlags, args)
return (S_OK, -1, _invokeex_(self, dispid, lcid, wFlags, args, nothing, nothing))
end

function _invokeex_(self::AbstractDynamicPolicy, dispid, lcid, wFlags, args, kwargs, serviceProvider)
try
