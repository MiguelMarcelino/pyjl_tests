...
return pythoncom.CoCreateInstance(pythoncom.CLSID_StdComponentCategoriesMgr, nothing, pythoncom.CLSCTX_INPROC_SERVER, pythoncom.IID_ICatRegister)
end
...
try
moduleName = splitext(os.path, win32api.FindFiles(append!([PROGRAM_FILE], ARGS)[1])[1][9])[1]
catch exn
...
flags["unattended"] = get(flags, "unattended", 0) || "--unattended" ∈ append!([PROGRAM_FILE], ARGS)
if unregisterInfo
return UnregisterInfoClasses(classes..., None = flags)
end
try
if unregister
UnregisterClasses(classes..., None = flags)
else
RegisterClasses(classes..., None = flags)
end
catch exn
 let exc = exn
if exc isa win32api.error
if flags["unattended"] || exc.winerror != winerror.ERROR_ACCESS_DENIED || sys.getwindowsversion()[1] < 5
error()
end
