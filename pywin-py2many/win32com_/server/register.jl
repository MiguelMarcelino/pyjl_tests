        ...
        if exn isa os.error
            ...
            try
                ...
                if exn isa os.error
                    throw(
                        RuntimeError(
                            "Can not locate the Python module \'win32com_.server.$(baseName)\'",
                        ),
                    )
                end
            end
...
function RegisterServer(
    ...
    if clsctx & pythoncom.CLSCTX_INPROC_SERVER
        if pythoncom.frozen
            ...
            loadername = joinpath(
                pythoncom_dir,
                "pythoncomloader$(sys.version_info[1])$(sys.version_info[2])$(suffix).dll",
            )
...
function ReExecuteElevated(flags)
    ...
    try
        ...
        try
            ...
            write(
                batf,
                "$(win32api.GetShortPathName(exe_to_run)) $(new_params) > \"$(outfile)\" 2>&1",
            )
        finally
            close(batf)
        end
...
if !(pythoncom.frozen)
    try
        win32api.RegQueryValue(
            ...
            "Component Categories\\$(CATID_PythonCOMServer)",
        )
    catch exn
        if exn isa win32api.error
            try
