...
try
interface = parse_interface_info(interface_name, fin)
finally
close(fin)
end
...
try
write(fout, "// This file implements the %s %s for Python.\n// Generated by makegw.py\n\n#include \"shell_pch.h\"\n" % (interface.name, desc))
write(fout, "#include \"Py%s.h\"\n\n// @doc - This file contains autoduck documentation\n" % interface.name)
if bMakeInterface
...
try
write(fout, "// This file declares the %s %s for Python.\n// Generated by makegw.py\n" % (interface.name, desc))
if bMakeInterface
...
write(f, "// *** The input argument %s of type \"%s\" was not processed ***\n//     Please check the conversion function is appropriate and exists!\n" % (arg.name, arg.raw_type))
write(f, "\t%s %s;\n\tPyObject *ob%s;\n" % (arg.type, arg.name, arg.name))
write(f, "\t// @pyparm <o Py%s>|%s||Description for %s\n" % (arg.type, arg.name, arg.name))
...
write(f, "\tif ( !PyArg_ParseTuple(args, \"%s:%s\"%s) )\n\t\treturn NULL;\n" % (formatChars, method.name, argsParseTuple))
if codePost
...
codeDecl = codeDecl + DeclareParseArgTupleInputConverter(argCvt)
 ...
 let why = exn
if why isa makegwparse.error_not_supported
write(f, "// *** The output argument %s of type \"%s\" was not processed ***\n//     %s\n" % (arg.name, arg.raw_type, why))
continue;
end
end
end
...
write(f, "\tPyObject *ob%s = PyObject_From%s(%s);\n" % (arg.name, arg.type, arg.name))
write(f, "\tif (ob%s==NULL) return MAKE_PYCOM_GATEWAY_FAILURE_CODE(\"%s\");\n" % (arg.name, method.name))
