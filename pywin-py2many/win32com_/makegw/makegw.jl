...
try
write(fout, "// This file implements the $(interface.name) $(desc) for Python.\n// Generated by makegw.py\n\n#include \"shell_pch.h\"\n")
write(fout, "#include \"Py$(interface.name).h\"\n\n// @doc - This file contains autoduck documentation\n")
if bMakeInterface
...
try
write(fout, "// This file declares the $(interface.name) $(desc) for Python.\n// Generated by makegw.py\n")
if bMakeInterface
...
write(f, "// *** The input argument $(arg.name) of type \"$(arg.raw_type)\" was not processed ***\n//     Please check the conversion function is appropriate and exists!\n")
write(f, "\t$(arg.type) $(arg.name);\n\tPyObject *ob$(arg.name);\n")
write(f, "\t// @pyparm <o Py$(arg.type)>|$(arg.name)||Description for $(arg.name)\n")
codePost = codePost + "\tif (bPythonIsHappy && !PyObject_As$(arg.type)( ob$(arg.name), &$(arg.name) )) bPythonIsHappy = FALSE;\n"
...
write(f, "\tif ( !PyArg_ParseTuple(args, \"$(formatChars):$(method.name)\"$(argsParseTuple)) )\n\t\treturn NULL;\n")
if codePost
...
codeDecl = codeDecl + DeclareParseArgTupleInputConverter(argCvt)
 ...
 let why = exn
if why isa makegwparse.error_not_supported
write(f, "// *** The output argument $(arg.name) of type \"$(arg.raw_type)\" was not processed ***\n//     $(why)\n")
continue;
end
end
end
...
for method in interface.methods
write(f, "\t{ \"$(method.name)\", Py$(interface.name)::$(method.name), 1 }, // @pymeth $(method.name)|Description of $(method.name)\n")
end
...
write(f, "\tPyObject *ob$(arg.name) = PyObject_From$(arg.type)($(arg.name));\n")
write(f, "\tif (ob$(arg.name)==NULL) return MAKE_PYCOM_GATEWAY_FAILURE_CODE(\"$(method.name)\");\n")
...
write(f, "\tif (!$(parseFn)(result, \"$(formatChars)\" $(argsParseTuple)))\n\t\treturn MAKE_PYCOM_GATEWAY_FAILURE_CODE(\"$(method.name)\");\n")
end
if codePost
write(f, "\tBOOL bPythonIsHappy = TRUE;\n")
write(f, codePost)
write(f, "\tif (!bPythonIsHappy) hr = MAKE_PYCOM_GATEWAY_FAILURE_CODE(\"$(method.name)\");\n")
end
