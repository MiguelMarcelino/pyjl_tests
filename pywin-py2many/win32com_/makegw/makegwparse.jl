...
#= Provide information about the C++ object used.
        ...
        Result is a tuple of (variableName, [DeclareType|None|""])
         =#
return (GetIndirectedArgName(self, self.builtinIndirection, self.arg.indirectionLevel + self.builtinIndirection), "%s %s" % (GetUnconstType(self), self.arg.name))
end
...
return "// @pyparm %s|%s||Description for %s" % (_GetPythonTypeDesc(self), self.arg.name, self.arg.name)
end
...
return (GetIndirectedArgName(self, 1, self.arg.indirectionLevel), "%s * %s" % (GetUnconstType(self), self.arg.name))
end
...
function GetFormatChar(self::ArgFormatterSimple)::Tuple<str>
return ConvertSimpleTypes[self.arg.type][2]
end

function _GetPythonTypeDesc(self::ArgFormatterSimple)::Tuple<str>
...
throw(error_not_supported("The type \'%s\' (%s) is unknown." % (arg.type, arg.name)))
end
end
end
...
regex
                    ...
                    Argument(arrayDecl::Int64, const_::Int64, good_interface_names, indirectionLevel::Int64, inout, name, raw_type, type_, unc_type, regex = compile("/\\* \\[([^\\]]*.*?)] \\*/[ \\t](.*[* ]+)(\\w+)(\\[ *])?[\\),]")) =
                        new(arrayDecl, const_, good_interface_names, indirectionLevel, inout, name, raw_type, type_, unc_type, regex)
end
...
regex
                    ...
                    Method(args::Vector, callconv, good_interface_names, name, result, regex = compile("virtual (/\\*.*?\\*/ )?(.*?) (.*?) (.*?)\\(\\w?")) =
