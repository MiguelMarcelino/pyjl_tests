...
mutable struct ArgFormatter <: AbstractArgFormatter
    ...
    declaredIndirection
    gatewayMode::Int64
    ArgFormatter(arg, builtinIndirection, declaredIndirection = 0, gatewayMode = 0) =
        new(arg, builtinIndirection, declaredIndirection, gatewayMode)
end
...
function GetIndirectedArgName(
    self::AbstractArgFormatter,
    indirectFrom,
    indirectionTo,
)::String
...
function GetInterfaceCppObjectInfo(self::AbstractArgFormatter)
    ...
    return (
        ...
        "$(GetUnconstType(self)) $(self.arg.name)",
    )
end
...
function GetFormatChar(self::AbstractArgFormatterSimple)::Tuple <
         str >
         return ConvertSimpleTypes[self.arg.type][2] end

function _GetPythonTypeDesc(self::AbstractArgFormatterSimple)::Tuple <
         str >
...
mutable struct Argument <: AbstractArgument
    ...
    unc_type
    regex
    Argument(good_interface_names, inout = nothing, const_ = 0, arrayDecl = 0) =
        new(good_interface_names, inout, const_, arrayDecl)
end
...
mutable struct Method <: AbstractMethod
    ...
    result
    regex
    Method(good_interface_names, name = nothing, args = []) =
        new(good_interface_names, name, args)
end
