...
throw(error("Calling the object natively did not yield the correct scode", com_exc))
end
exc = com_exc.excepinfo
if !(exc) || exc[end] != winerror.E_UNEXPECTED
throw(error("The scode element of the exception tuple did not yield the correct scode", com_exc))
end
if exc[3] != "Not today"
throw(error("The description in the exception tuple did not yield the correct string", com_exc))
end
end
end
end
...
throw(error("The hresult was not E_FAIL for an internal error", com_exc))
end
...
throw(error("Calling the object via IDispatch did not yield the correct scode", com_exc))
end
exc = com_exc.excepinfo
if !(exc) || exc[end] != winerror.E_UNEXPECTED
throw(error("The scode element of the exception tuple did not yield the correct scode", com_exc))
...
if exc[2] != "source"
throw(error("The source in the exception tuple did not yield the correct string", com_exc))
end
if exc[3] != "😀"
throw(error("The description in the exception tuple did not yield the correct string", com_exc))
end
if exc[4] != "helpfile"
throw(error("The helpfile in the exception tuple did not yield the correct string", com_exc))
end
if exc[5] != 1
throw(error("The help context in the exception tuple did not yield the correct string", com_exc))
end
end
end
end
...
@assert(startswith(handler.last_record, "pythoncom error: Unexpected exception in gateway method \'Commit\'"))
reset(handler)
com_server = Dispatch(wrap(TestServer()))
try
Commit(com_server, 0)
