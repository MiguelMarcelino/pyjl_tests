...
_public_methods_
            ...
            TestDataObject(bytesval, _com_interfaces_::Vector = [pythoncom.IID_IDataObject], _public_methods_ = IDataObject_Methods) = begin
...
OleSetClipboard(do_)
assertTrue(self, OleIsCurrentClipboard(do_))
end
...
assertEqual(self, got, expected)
got = GetClipboardData(win32con.CF_UNICODETEXT)
assertEqual(self, got, "Hello from Python")
CloseClipboard()
end
...
stg = GetData(do_, cf)
got = stg.data
assertTrue(self, got, b"Hello again!\x00")
end
...
do_ = WrapCOMObject(do_)
OleSetClipboard(do_)
assertEqual(self, num_do_objects, 1)
do_ = nothing
OleFlushClipboard()
assertEqual(self, num_do_objects, 0)
end
...
function testIsCurrentClipboard(self::ClipboardTester)
    ...
    do_ = WrapCOMObject(do_)
    OleSetClipboard(do_)
    @test OleIsCurrentClipboard(do_)
end
