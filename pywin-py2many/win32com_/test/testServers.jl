...
win32com_.test.util.assertRaisesCOM_HRESULT(self, winerror.DISP_E_TYPEMISMATCH, interp.Eval, 2)
end

function testInproc(self::AbstractInterpCase)
interp = win32com_.client.dynamic.Dispatch("Python.Interpreter", clsctx = pythoncom.CLSCTX_INPROC)
_testInterp(self, interp)
end

function testLocalServer(self::AbstractInterpCase)
interp = win32com_.client.dynamic.Dispatch("Python.Interpreter", clsctx = pythoncom.CLSCTX_LOCAL_SERVER)
_testInterp(self, interp)
end
