...
mutable struct LockBytes <: AbstractLockBytes
    ...
    LockBytes(data = str2bytes(data), ctime = now, mtime = now, atime = now) =
        new(data, ctime, mtime, atime)
end
...
mutable struct OleClientSite <: AbstractOleClientSite
    ...
    _public_methods_::Vector{String}
    data::String
    OleClientSite(data = "", IPersistStorage = nothing, IStorage = nothing) =
        new(data, IPersistStorage, IStorage)
end
