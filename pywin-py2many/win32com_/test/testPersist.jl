...
_public_methods_::Vector{String}
                    ...
                    LockBytes(atime, ctime, data, mtime, _com_interfaces_::Vector = [pythoncom.IID_ILockBytes], _public_methods_::Vector{String} = ["ReadAt", "WriteAt", "Flush", "SetSize", "LockRegion", "UnlockRegion", "Stat"]) =
...
self.data = self.data + str2bytes(repeat("\000",(size - length(self.data))))
else
...
data::String
                    ...
                    OleClientSite(IPersistStorage, IStorage, _com_interfaces_::Vector = [axcontrol.IID_IOleClientSite], _public_methods_::Vector{String} = ["SaveObject", "GetMoniker", "GetContainer", "ShowObject", "OnShowWindow", "RequestNewObjectLayout"], data::String = "") =
...
stcom = StgCreateDocfileOnILockBytes(lbcom, ((storagecon.STGM_DIRECT | storagecon.STGM_CREATE) | storagecon.STGM_READWRITE) | storagecon.STGM_SHARE_EXCLUSIVE, 0)
ocs = OleClientSite()
ocscom = wrap(ocs, axcontrol.IID_IOleClientSite)
oocom = OleCreate("{00020906-0000-0000-C000-000000000046}", axcontrol.IID_IOleObject, 0, (0,), ocscom, stcom)
mf = GetMainFrame()
hwnd = GetSafeHwnd(mf)
