...
function CallbackResultOneByRef(self::AbstractTestObject, intVal)
    return (intVal, intVal + 1)
end

function CallbackVoidTwoByRef(self::AbstractTestObject, int1, int2)::Tuple
    return (int1 + int2, int1 - int2)
end

function CallbackString(self::AbstractTestObject, strVal)
    return (0, strVal + " has visited Python")
end

function CallbackArrayResult(self::AbstractTestObject, arrayVal)::Vector
    ret = []
    for i in arrayVal
        push!(ret, i + 1)
    end
...
function CallbackResultOneByRefButReturnNone(self::AbstractTestObject, intVal)
    return
end

function CallbackVoidOneByRefButReturnNone(self::AbstractTestObject, intVal)
    return
end
...
function TestVB(vbtest, bUseGenerated)
    ...
    if vbtest.VariantProperty != (1.0, 2.0, 3.0)
        throw(
            error(
                "Could not set the variant property to an array of floats correctly - \'$(vbtest.VariantProperty)\'.",
            ),
        )
    end
...
function _DoTestCollection(vbtest, col_name, expected)
    ...
    if check != collect(expected)
        throw(
            error(
                "Collection 2nd time around $(col_name) didn\'t have $(expected) (had $(check))",
            ),
        )
    end
    ...
    if check != collect(expected)
        throw(
            error(
                "Collection iterator $(col_name) didn\'t have $(expected) 2nd time around (had $(check))",
            ),
        )
    end
    check = []
    ...
    if check != []
        throw(
            error(
                "2nd time around Collection iterator $(col_name) wasn\'t empty (had $(check))",
            ),
        )
    end
    c = getfield(vbtest, :col_name)
    if length(c) != _getcount(c)
        throw(
            error(
                "Collection $(col_name) __len__($(length(c))) wasn\'t==Count($(_getcount(c)))",
            ),
        )
    end
    c = getfield(vbtest, :col_name)
...
function _DoTestArray(vbtest, data, expected_exception = nothing)
    ...
    if got != data
        throw(
            error("Could not set the array data correctly - got $(got), expected $(data)"),
        )
    end
end
