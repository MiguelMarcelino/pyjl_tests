...
throw(error("Could not set the variant property to an array of floats correctly - \'%s\'." % (vbtest.VariantProperty,)))
end
...
throw(error("Could not pass an int VARIANT byref:" * string(IncrementVariantParam(vbtest, 1))))
end
...
if check !== collect(expected)
throw(error("Collection 2nd time around %s didn\'t have %r (had %r)" % (col_name, expected, check)))
end
...
if check !== collect(expected)
throw(error("Collection iterator %s didn\'t have %r 2nd time around (had %r)" % (col_name, expected, check)))
end
check = []
for item in i
push!(check, item)
...
if check !== []
throw(error("2nd time around Collection iterator %s wasn\'t empty (had %r)" % (col_name, check)))
end
c = getfield(vbtest, :col_name)
if length(c) != _getcount(c)
throw(error("Collection %s __len__(%r) wasn\'t==Count(%r)" % (col_name, length(c), _getcount(c))))
end
c = getfield(vbtest, :col_name)
check = []
for i in 0:_getcount(c) - 1
push!(check, c[i + 1])
end
if check !== collect(expected)
throw(error("Collection %s didn\'t have %r (had %r)" % (col_name, expected, check)))
...
if got != data
throw(error("Could not set the array data correctly - got %r, expected %r" % (got, data)))
end
end
...
throw(error("The safe array data was not what we expected - got " * string(resultData)))
end
if testData !== collect(byRefParam)
throw(error("The safe array data was not what we expected - got " * string(byRefParam)))
end
...
if s.sub_val.array_val[1].int_val != 0 || string(s.sub_val.array_val[1].str_val) != "zero"
