...
function TestConstant(constName, pyConst)
    ...
    if comConst != pyConst
        throw(
            error(
                "Constant value wrong for $(constName) - got $(comConst), wanted $(pyConst)",
            ),
        )
    end
end
...
function OnFireWithNamedParams(
    self::AbstractNewStyleRandomEventHandler,
    no,
    a_bool,
    out1,
    out2,
...
function TestGenerated()
    ...
    if !isa(i1, DispatchBaseClass) || !isa(i2, DispatchBaseClass)
        throw(
            error(
                "GetMultipleInterfaces did not return instances - got \'$(i1)\', \'$(i2)\'",
            ),
        )
    end
    #Delete Unsupported
    ...
    for i = 0:49
        ...
        try
            ...
            if ret != (num + 1)
                throw(
                    error(
                        "Random access into element $(num) failed - return was $(repr(ret))",
                    ),
                )
            end
        catch exn
