...
dispatch = _GetGoodDispatch("Word.Application")
...
word = CDispatch(dispatch, olerepr)
dispatch = nothing
typeinfo = nothing
attr = nothing
olerepr = nothing
...
try
word = EnsureDispatch("Word.Application.8")
TestWord8(word)
catch exn
 let e = exn
if e isa Exception
...
if Range(xl, "A1:C3").Value != ((Unicode("Hi 0"), 2, 3), (Unicode("x"), Unicode("Hi 1"), Unicode("z")), (3, 2, Unicode("Hi 2")))
throw(error("square cell range failed"))
end
Range(xl, "A1:C3").Value = ((3, 2, 1), ("x", "y", "z"), (1, 2, 3))
if Range(xl, "A1:C3").Value != ((3, 2, 1), (Unicode("x"), Unicode("y"), Unicode("z")), (1, 2, 3))
...
Cells(xl, 6, 2).Value = MakeTime(time())
...
mod = EnsureModule("{00020813-0000-0000-C000-000000000046}", 0, 1, 2, 1)
...
try
mod = EnsureModule("{00020813-0000-0000-C000-000000000046}", 9, 1, 0)
