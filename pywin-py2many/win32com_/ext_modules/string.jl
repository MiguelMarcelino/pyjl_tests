...
printable -- a string containing all ASCII characters considered printable
 ...
 =#
export ascii_letters, ascii_lowercase, ascii_uppercase, capwords, digits, hexdigits, octdigits, printable, punctuation, whitespace, Formatter, Template
import _string
whitespace = " \t\n\r\v\f"
...
function _invalid(self::AbstractTemplate, mo)
i = start(mo, "invalid")
...
function convert(mo::AbstractTemplate)::String
...
function vformat(self::AbstractFormatter, format_string, args, kwargs)
used_args = set()
...
function _vformat(self::AbstractFormatter, format_string, args, kwargs, used_args, recursion_depth, auto_arg_index = 0)::Tuple
...
format_spec, auto_arg_index = _vformat(self, format_spec, args, kwargs, used_args, recursion_depth - 1)
...
function get_value(self::AbstractFormatter, key, args, kwargs)
if isa(key, int)
return args[key + 1]
else
return kwargs[key + 1]
...
function check_unused_args(self::AbstractFormatter, used_args, args, kwargs)
#= pass =#
end

function format_field(self::AbstractFormatter, value, format_spec)
return value
end

function convert_field(self::AbstractFormatter, value, conversion)::String
...
function parse(self::AbstractFormatter, format_string)
return _string.formatter_parser(format_string)
end

function get_field(self::AbstractFormatter, field_name, args, kwargs)::Tuple
