...
function _makeDocString(s::AbstractLazyDispatchItem)
...
NoTranslateTypes = [pythoncom.VT_BOOL, pythoncom.VT_CLSID, pythoncom.VT_CY, pythoncom.VT_DATE, pythoncom.VT_DECIMAL, pythoncom.VT_EMPTY, pythoncom.VT_ERROR, pythoncom.VT_FILETIME, pythoncom.VT_HRESULT, pythoncom.VT_I1, pythoncom.VT_I2, pythoncom.VT_I4, pythoncom.VT_I8, pythoncom.VT_INT, pythoncom.VT_NULL, pythoncom.VT_R4, pythoncom.VT_R8, pythoncom.VT_NULL, pythoncom.VT_STREAM, pythoncom.VT_UI1, pythoncom.VT_UI2, pythoncom.VT_UI4, pythoncom.VT_UI8, pythoncom.VT_UINT, pythoncom.VT_VOID]
NoTranslateMap = Dict()
...
resultDoc
            ...
            MapEntry(desc_or_id = nothing, names = 0, doc = names, resultCLSID = doc, resultDoc = resultCLSID, hidden = resultDoc, resultDocumentation = 0, wasProperty = hidden) = begin
...
desc = desc_or_id
                ...
                new(desc_or_id , names , doc , resultCLSID , resultDoc , hidden , resultDocumentation , wasProperty )
            end
end
...
typename::String
            ...
            OleItem(doc = doc, bWritten = 0, bIsDispatch = 0, bIsSink = 0, clsid = nothing, co_class = nothing, typename::String = "OleItem") = begin
                if doc
python_name = MakePublicAttributeName(doc[0])
else
python_name = nothing
end
                new(doc , bWritten , bIsDispatch , bIsSink , clsid , co_class , typename)
            end
end
...
typename::String
            ...
            DispatchItem(typeinfo = nothing, attr = nothing, doc = nothing, bForUser = 1, propMap = Dict(), propMapGet = Dict(), propMapPut = Dict(), mapFuncs = Dict(), defaultDispatchName = nothing, hidden = 0, typename::String = "DispatchItem") = begin
                OleItem(doc)
if typeinfo
Build(typeinfo, attr, bForUser)
end
                new(typeinfo , attr , doc , bForUser , propMap , propMapGet , propMapPut , mapFuncs , defaultDispatchName , hidden , typename)
            end
end
function _propMapPutCheck_(self::AbstractDispatchItem, key::String, item::MapEntry)
...
function _propMapGetCheck_(self::AbstractDispatchItem, key::String, item::MapEntry)
ins, outs, opts = CountInOutOptArgs(self, item.desc[3])
if ins > 0
...
function _AddVar_(self::AbstractDispatchItem, typeinfo, vardesc, bForUser)
...
function CountInOutOptArgs(self::AbstractDispatchItem, argTuple)
...
function MakeDispatchFuncMethod(self::AbstractDispatchItem, entry, name, bMakeClass = 1)::Vector
fdesc = entry.desc
doc = entry.doc
names = entry.names
ret = []
...
s = "$(linePrefix)$(id)$(fdesc[5])$(retDesc)$(argsDesc)$(_BuildArgList(fdesc, names)))"
elseif rd ∈ [pythoncom.VT_DISPATCH, pythoncom.VT_UNKNOWN]
s = "$(linePrefix)$(id)$(fdesc[5])$(retDesc)$(repr(argsDesc))$(_BuildArgList(fdesc, names)))
"
s = s + "$(linePrefix)	if ret is not None:
"
if rd == pythoncom.VT_UNKNOWN
s = s + "$(linePrefix)		# See if this IUnknown is really an IDispatch
"
s = s + "$(linePrefix)		try:
"
s = s + "$(linePrefix)			ret = ret.QueryInterface(pythoncom.IID_IDispatch)
"
s = s + "$(linePrefix)		except pythoncom.error:
"
s = s + "$(linePrefix)			return ret
"
end
s = s + "$(linePrefix)$(repr(name))$(resclsid))
"
s = s + "	return ret"
elseif rd == pythoncom.VT_BSTR
s = "$(linePrefix)	# Result is a Unicode object
"
s = s + "$(linePrefix)$(id)$(fdesc[5])$(retDesc)$(repr(argsDesc))$(_BuildArgList(fdesc, names)))"
end
end
if s === nothing
s = "$(linePrefix)$(id)$(fdesc[5])$(retDesc)$(argsDesc)$(repr(name))$(resclsid)$(_BuildArgList(fdesc, names)))"
end
push!(ret, s)
push!(ret, "")
return ret
...
function MakeVarArgsFuncMethod(self::AbstractDispatchItem, entry, name, bMakeClass = 1)::Vector
fdesc = entry.desc
names = entry.names
doc = entry.doc
ret = []
...
push!(ret, s * string(entry.dispid) + "$(invoketype)$(names[1])')")
push!(ret, "")
return ret
end
...
meth_list = append!(append!(collect(values(self.mapFuncs)), collect(values(self.propMapGet))), collect(values(self.propMapPut)))
...
typename::String
            ...
            LazyDispatchItem(attr, doc, clsid = attr[0], typename::String = "LazyDispatchItem") = begin
                DispatchItem(nothing, attr, doc, 0)
                new(attr, doc, clsid , typename)
            end
end

typeSubstMap = Dict(pythoncom.VT_INT => pythoncom.VT_I4, pythoncom.VT_UINT => pythoncom.VT_UI4, pythoncom.VT_HRESULT => pythoncom.VT_I4)
function _ResolveType(typerepr::AbstractLazyDispatchItem, itypeinfo)::Tuple
...
if was_user && subrepr ∈ [pythoncom.VT_DISPATCH, pythoncom.VT_UNKNOWN, pythoncom.VT_RECORD]
...
function _BuildArgList(fdesc::AbstractLazyDispatchItem, names)::String
...
function demunge_leading_underscores(className::AbstractLazyDispatchItem)::Any
i = 0
...
function MakePublicAttributeName(className::AbstractLazyDispatchItem, is_global = false)::Any
...
function MakeDefaultArgRepr(defArgVal::AbstractLazyDispatchItem)::String
try
inOut = defArgVal[2]
catch exn
...
function BuildCallList(fdesc::AbstractLazyDispatchItem, names, defNamedOptArg, defNamedNotOptArg, defUnnamedArg, defOutArg, is_comment = false)::String
