...
resultDoc
            ...
            MapEntry(desc_or_id, names = nothing, doc = nothing, resultCLSID = pythoncom.IID_NULL, resultDoc = nothing, hidden = 0) = begin
...
typename::String
            ...
            DispatchItem(typeinfo = nothing, attr = nothing, doc = nothing, bForUser = 1, typename::String = "DispatchItem") = begin
                OleItem(doc)
if typeinfo
...
s = "%s\treturn self._oleobj_.InvokeTypes(%d, LCID, %s, %s, %s%s)" % (linePrefix, id, fdesc[5], retDesc, argsDesc, _BuildArgList(fdesc, names))
elseif rd ∈ [pythoncom.VT_DISPATCH, pythoncom.VT_UNKNOWN]
s = "%s\tret = self._oleobj_.InvokeTypes(%d, LCID, %s, %s, %s%s)\n" % (linePrefix, id, fdesc[5], retDesc, repr(argsDesc), _BuildArgList(fdesc, names))
...
s = s + ("%s\treturn self._oleobj_.InvokeTypes(%d, LCID, %s, %s, %s%s)" % (linePrefix, id, fdesc[5], retDesc, repr(argsDesc), _BuildArgList(fdesc, names)))
end
end
if s === nothing
s = "%s\treturn self._ApplyTypes_(%d, %s, %s, %s, %s, %s%s)" % (linePrefix, id, fdesc[5], retDesc, argsDesc, repr(name), resclsid, _BuildArgList(fdesc, names))
end
push!(ret, s)
push!(ret, "")
return ret
...
push!(ret, s * string(entry.dispid) + (",0,%d,1)+args)),\'%s\')" % (invoketype, names[1])))
push!(ret, "")
return ret
end
