...
function __repr__(self::AbstractCDispatch)::String
    return "<COMObject $(self._username_)>"
end
...
function _make_method_(self::AbstractCDispatch, name)
    ...
    try
        codeObject = compile(methodCode, "<COMObject $(self._username_)>", "exec")
        tempNameSpace = Dict()
...
function __LazyMap__(self::AbstractCDispatch, attr)::Int64
    try
        if _LazyAddAttr_(self, attr)
            debug_attr_print("$(self._username_).__LazyMap__($(attr)) added something")
            return 1
        end
    catch exn
        if exn isa AttributeError
...
function __AttrToID__(self::AbstractCDispatch, attr)
    debug_attr_print(
        "Calling GetIDsOfNames for property $(attr) in Dispatch container $(self._username_)",
    )
    ...
    if retEntry !== nothing
        try
            ...
            if exn isa (KeyError, AttributeError)
                debug_attr_print("Attribute $(attr) not in cache")
            end
        end
    end
    if retEntry !== nothing
        invoke_type = _GetDescInvokeType(retEntry, pythoncom.INVOKE_PROPERTYGET)
        debug_attr_print("Getting property Id 0x$(retEntry.dispid) from OLE object")
        try
...
function __setattr__(self::AbstractCDispatch, attr, value)
    ...
    debug_attr_print(
        "SetAttr called for $(self._username_).$(attr)=$(repr(value)) on DispatchContainer",
    )
    if self._olerepr_
    ...
    if self._oleobj_
        ...
        if entry !== nothing
            try
                ...
                debug_attr_print(
                    "__setattr__ property $(attr) (id=0x$(entry.dispid)) in Dispatch container $(self._username_)",
                )
                return
            catch exn
                if exn isa pythoncom.com_error
    ...
    throw(AttributeError("Property \'$(self._username_).$(attr)\' can not be set."))
end
