...
IDispatch = CoCreateInstance(IDispatch, nothing, clsctx, pythoncom.IID_IDispatch)
end
end
else
...
function Dispatch(IDispatch, userName = nothing, createClass = nothing, typeinfo = nothing, UnicodeToString = nothing, clsctx = pythoncom.CLSCTX_SERVER)
...
function DumbDispatch(IDispatch, userName = nothing, createClass = nothing, UnicodeToString = nothing, clsctx = pythoncom.CLSCTX_SERVER)
...
__class__
            ...
            CDispatch(IDispatch, olerepr, userName = nothing, UnicodeToString = nothing, lazydata = nothing) = begin
                @assert(UnicodeToString === nothing)
if userName === nothing
userName = "<unknown>"
end
...
function _ApplyTypes_(self::CDispatch, dispid, wFlags, retType, argTypes, user, resultCLSID)::Tuple
...
function _wrap_dispatch_(self::CDispatch, ob, userName = nothing, returnCLSID = nothing, UnicodeToString = nothing)
@assert(UnicodeToString === nothing)
return Dispatch(ob, userName)
end

function _get_good_single_object_(self::CDispatch, ob, userName = nothing, ReturnCLSID = nothing)
...
function _get_good_object_(self::CDispatch, ob, userName = nothing, ReturnCLSID = nothing)::Tuple
