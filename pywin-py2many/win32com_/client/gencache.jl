...
function _SaveDicts()
if is_readonly
throw(RuntimeError("Trying to write to a readonly gencache ('')!"))
end
...
#= Given the clsid, lcid, major and  minor for a type lib, return
    the file name (no extension) providing this support.
     =#
return upper(string(clsid))[2:-1] + "$(lcid)$(major)$(minor)"
end
...
@assert(!(is_readonly))
try
mkpath(os)(win32com_.__gen_path__)
catch exn
if exn isa os.error
#= pass =#
end
...
if exn isa KeyError
println("$(info) is not being remembered!")
end
end
...
module_ = nothing
try
tlbAttr = GetLibAttr(pythoncom.LoadRegTypeLib(typelibCLSID, major, minor, lcid))
...
tlbAttributes = GetLibAttr(pythoncom.LoadRegTypeLib(typelibCLSID, major, minor, lcid))
catch exn
if exn isa pythoncom.com_error
bValidateFile = 0
end
...
if module_ !== nothing && bValidateFile
@assert(!(is_readonly))
filePathPrefix = "$(GetGeneratePath())$(GetGeneratedFileName(typelibCLSID, lcid, major, minor))"
...
println("$(info)$(sys.exc_info()[1])$(sys.exc_info()[2])")
end
end
if verbose && length(infos)
println("Done.")
...
mod = GetModuleForTypelib(typelibCLSID, lcid, major, minor)
println("$(mod.__doc__)$(typelibCLSID)")
end
end
...
try
opts, args = getopt()(append!([PROGRAM_FILE], ARGS)[2:end], "qrd")
catch exn
 let message = exn
