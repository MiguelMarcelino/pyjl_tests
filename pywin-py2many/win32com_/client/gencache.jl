...
function _SaveDicts()
    if is_readonly
        throw(
            RuntimeError(
                "Trying to write to a readonly gencache (\'$(win32com_.__gen_path__)\')!",
            ),
        )
    end
...
function GetGeneratedFileName(clsid, lcid, major, minor)::Any
    #= Given the clsid, lcid, major and  minor for a type lib, return
        the file name (no extension) providing this support.
         =#
    return upper(string(clsid))[1:end-1] + "x$(lcid)x$(major)x$(minor)"
end
    ...
    try
        ...
        if module_ !== nothing && bValidateFile
            ...
            try
                ...
                if typLibPath[end] == "\0"
                    typLibPath = typLibPath[begin:end-1]
                end
                suf = (
...
function _GetModule(fname)
    ...
    mod_name = "win32com_.gen_py.$(fname)"
    ...
    for info in infos
        ...
        try
            ...
            @printf(
                ...
                (info, sys.exc_info()[1], sys.exc_info()[2])
            )
        end
    end
    if verbose && length(infos)
