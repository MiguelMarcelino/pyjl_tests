...
return "(%s, %d, %s, %s, \"%s\", %s)" % (dispid, wFlags, retType[begin:2], argTypes, user, resultCLSID)
end
...
typename::String
            ...
            RecordItem(typeInfo, typeAttr, doc = nothing, bForUser = 1, order::Int64 = 9, typename::String = "RECORD") = begin
...
typename::String
            ...
            AliasItem(typeinfo, attr, doc = nothing, bForUser = 1, order::Int64 = 2, typename::String = "ALIAS") = begin
                build.OleItem.__init__(self, doc)
...
typename::String
            ...
            EnumerationItem(typeinfo, attr, doc = nothing, bForUser = 1, order::Int64 = 1, typename::String = "ENUMERATION") = begin
                build.OleItem.__init__(self, doc)
for j in 0:attr[7]
...
write(stream, "\t\t\tcpc=oobj._oleobj_.QueryInterface(pythoncom.IID_IConnectionPointContainer)")
write(stream, "\t\t\tcp=cpc.FindConnectionPoint(self.CLSID_Sink)")
write(stream, "\t\t\tcookie=cp.Advise(win32com_.server.util.wrap(self, usePolicy=EventHandlerPolicy))")
...
write(stream, "\t\t\tcp,cookie,self._olecp,self._olecp_cookie = self._olecp,self._olecp_cookie,None,None")
...
write(stream, "$(("#\tdef " * methName * "(self" + BuildCallList(fdesc, entry.names, "defaultNamedOptArg", "defaultNamedNotOptArg", "defaultUnnamedArg", "pythoncom.Missing", true)))):")
...
sort(names)
specialItems = Dict("count" => nothing, "item" => nothing, "value" => nothing, "_newenum" => nothing)
itemCount = nothing
for name in names
entry = self.mapFuncs[name + 1]
@assert(entry.desc.desckind == pythoncom.DESCKIND_FUNCDESC)
dispid = entry.desc.memid
if entry.desc.wFuncFlags & pythoncom.FUNCFLAG_FRESTRICTED && dispid != pythoncom.DISPID_NEWENUM
continue;
end
...
write(stream, "$(name) is actually a property, but must be used as a method to correctly pass the arguments")
end
...
argDesc = ()
mapEntry = MakeMapLineEntry(details.memid, pythoncom.DISPATCH_PROPERTYGET, resultDesc, argDesc, key, GetResultCLSIDStr(entry))
...
mapEntry = MakeMapLineEntry(details[1], pythoncom.DISPATCH_PROPERTYGET, resultDesc, argDesc, key, GetResultCLSIDStr(entry))
if details.memid == pythoncom.DISPID_VALUE
lkey = "value"
elseif details.memid == pythoncom.DISPID_NEWENUM
lkey = "_newenum"
...
write(stream, "$(MakePublicAttributeName(key))\" : (($(details[1]), 0),($(pythoncom.DISPATCH_PROPERTYPUT)")
end
end
...
write(stream, "$(MakePublicAttributeName(key))\": (($(details[1]), 0),$(details[5])")
end
end
write(stream, "\t}")
...
write(stream, "\t#This class has Item property/method which allows indexed access with the object[key] syntax.")
...
write(stream, "\t#This class has a __len__ - this is needed so \'if object:\' always returns TRUE.")
...
typename::String
            ...
            CoClassItem(typeinfo, attr, doc = nothing, sources = [], interfaces = [], bForUser = 1, order::Int64 = 5, typename::String = "COCLASS") = begin
                build.OleItem.__init__(self, doc)
...
write(stream, "$(ref.python_name) = sys.modules[\'$(generator.base_mod_name)\'].$(ref.python_name)")
ref.bWritten = 1
end
end
try
...
bUnicodeToString
            ...
            Generator(typelib, sourceFilename, progressObject, bBuildHidden = 1, bUnicodeToString = nothing) = begin
...
ret = []
for i in 0:GetTypeInfoCount(self.typelib) - 1
...
push!(child_infos, (info, refAttr.typekind, refType, GetDocumentation(refType, -1), refAttr, flags))
end
newItem = CoClassItem(info, attr, doc)
return (newItem, child_infos)
end

function _Build_CoClassChildren(self::Generator, coclass, coclass_info, oleItems, vtableItems)
sources = Dict()
interfaces = Dict()
for (info, info_type, refType, doc, refAttr, flags) in coclass_info
if refAttr.typekind == pythoncom.TKIND_DISPATCH || refAttr.typekind == pythoncom.TKIND_INTERFACE && refAttr[12] & pythoncom.TYPEFLAG_FDISPATCHABLE
clsid = refAttr[1]
if clsid ∈ oleItems
...
oleItem = nothing
vtableItem = nothing
if infotype === pythoncom.TKIND_DISPATCH || infotype === pythoncom.TKIND_INTERFACE && attr[12] & pythoncom.TYPEFLAG_FDISPATCHABLE
...
SetDescription(self.progress, "Generating...", (length(oleItems) + length(enumItems)) + length(vtableItems))
if enumItems
...
write(stream, "win32com_.client.CLSIDToClass.RegisterCLSIDsFromDict( CLSIDToClassMap )")
...
major = la[4]
minor = la[5]
self.base_mod_name = ("win32com_.gen_py." + string(clsid)[2:-1]) + ("x%sx%sx%s" % (lcid, major, minor))
try
oleItems = OrderedDict()
vtableItems = OrderedDict()
...
_Build_CoClassChildren(self, coClassItem, child_infos, oleItems, vtableItems)
has_break = true
break;
end
end
