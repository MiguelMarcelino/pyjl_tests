...
function CastTo(ob, target, typelib = nothing)
    ...
    if typelib !== nothing
        ...
        if !hasfield(typeof(mod), :target)
            throw(
                ValueError(
                    "The interface name \'$(target)\' does not appear in the specified library $(typelib.ver_desc)",
                ),
            )
        end
    elseif hasfield(typeof(target), :index)
        ...
        if target_clsid === nothing
            throw(
                ValueError(
                    "The interface name \'$(target)\' does not appear in the same library as object \'$(ob)\'",
                ),
            )
        end
...
function Record(name, object)
    ...
    try
        ...
        if exn isa KeyError
            throw(
                ValueError(
                    "The structure \'$(name)\' is not defined in module \'$(package)\'",
                ),
            )
        end
    end
...
function __repr__(self::AbstractDispatchBaseClass)
    ...
    return "<$(mod_name).$(self.__class__.__name__) instance at 0x$(id(self))>"
end
...
function __getattr__(self::AbstractDispatchBaseClass, attr)
    ...
    if args === nothing
        throw(AttributeError("\'$(repr(self))\' object has no attribute \'$(attr)\'"))
    end
...
function __repr__(self::AbstractCoClassBaseClass)
    return "<win32com_.gen_py.$(__doc__).$(self.__class__.__name__)>"
end
...
function __repr__(self::AbstractVARIANT)
    return "win32com_.client.VARIANT($(self.varianttype), $(self._value))"
end
