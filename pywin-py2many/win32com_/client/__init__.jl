...
function __WrapDispatch(dispatch, userName = nothing, resultCLSID = nothing, typeinfo = nothing, UnicodeToString = nothing, clsctx = pythoncom.CLSCTX_SERVER, WrapperClass = nothing)
#= 
...
function Dispatch(dispatch, userName = nothing, resultCLSID = nothing, typeinfo = nothing, UnicodeToString = nothing, clsctx = pythoncom.CLSCTX_SERVER)
...
function DispatchEx(clsid, machine = nothing, userName = nothing, resultCLSID = nothing, typeinfo = nothing, UnicodeToString = nothing, clsctx = nothing)
...
dispatch = CoCreateInstanceEx(clsid, nothing, clsctx, serverInfo, (pythoncom.IID_IDispatch,))[1]
...
mod = nothing
if typelib !== nothing
mod = MakeModuleForTypelib(typelib.clsid, typelib.lcid, parse(Int, typelib.major), parse(Int, typelib.minor))
if !hasfield(typeof(mod), :target)
throw(ValueError("The interface name \'%s\' does not appear in the specified library %r" % (target, typelib.ver_desc)))
end
...
throw(ValueError("The interface name \'%s\' does not appear in the same library as object \'%r\'" % (target, ob)))
end
...
package = GetModuleForTypelib(module_.CLSID, module_.LCID, module_.MajorVersion, module_.MinorVersion)
try
struct_guid = package.RecordMap[name + 1]
catch exn
if exn isa KeyError
throw(ValueError("The structure \'%s\' is not defined in module \'%s\'" % (name, package)))
end
end
return GetRecordFromGuids(module_.CLSID, module_.MajorVersion, module_.MinorVersion, module_.LCID, struct_guid)
end
...
function _ApplyTypes_(self::DispatchBaseClass, dispid, wFlags, retType, argTypes, user, resultCLSID)
...
function _get_good_single_object_(self::DispatchBaseClass, obj, obUserName = nothing, resultCLSID = nothing)
return _get_good_single_object_(obj, obUserName, resultCLSID)
end

function _get_good_object_(self::DispatchBaseClass, obj, obUserName = nothing, resultCLSID = nothing)
