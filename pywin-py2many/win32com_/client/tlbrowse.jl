...
typekindmap = Dict(pythoncom.TKIND_ENUM => "Enumeration", pythoncom.TKIND_RECORD => "Record", pythoncom.TKIND_MODULE => "Module", pythoncom.TKIND_INTERFACE => "Interface", pythoncom.TKIND_DISPATCH => "Dispatch", pythoncom.TKIND_COCLASS => "CoClass", pythoncom.TKIND_ALIAS => "Alias", pythoncom.TKIND_UNION => "Union")
...
function _GetRealMemberPos(self::AbstractTypeBrowseDialog, pos)
pos = GetCurSel(self.memberlb)
if pos >= self.attr[8]
...
function GetTemplate(self::AbstractTypeBrowseDialog)::Vector
