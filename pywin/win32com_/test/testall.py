...
# We'd prefer the win32com_ namespace to be the parent of __file__ - ie, our source-tree,
...
def CleanGenerated():
    import win32com_, shutil

    if os.path.isdir(win32com_.__gen_path__):
        if verbosity > 1:
            print("Deleting files from %s" % (win32com_.__gen_path__))
        shutil.rmtree(win32com_.__gen_path__)
    import win32com_.client.gencache

    win32com_.client.gencache.__init__()  # Reset
...
class PippoTest(TestCase):
    def testit(self):
        ...
        from win32com_.test import pippo_server
...
# This is a list of "win32com_.test.???" module names, optionally with a
...
unittest_other_modules = [
    # Level 1 tests.
    """win32com_.directsound.test.ds_test
        """.split(),
    # Level 2 tests.
    [],
    # Level 3 tests.
