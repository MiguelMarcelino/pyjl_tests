...
def _find_localserver_module():
    ...
    path = win32com_.server.__path__[0]
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                "Can not locate the Python module 'win32com_.server.%s'" % baseName
            )
    return pyfile
...
def RegisterServer(
    ...
    if clsctx & pythoncom.CLSCTX_LOCAL_SERVER:
        if pythoncom.frozen:
            ...
            # 'python.exe win32com_\server\localserver.py {clsid}"
...
def ReExecuteElevated(flags):
    from win32com_.shell.shell import ShellExecuteEx
    from win32com_.shell import shellcon
