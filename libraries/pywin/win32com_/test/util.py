...
def RegisterPythonServer(filename, progids=None, verbose=0):
    ...
    try:
        from win32com_.shell.shell import IsUserAnAdmin
    except ImportError:
        print("Can't import win32com_.shell - no idea if you are an admin or not?")
        is_admin = False
    else:
        try:
...
# Utilities to set the win32com_ logger to something what just captures
...
def setup_test_logger():
    old_log = getattr(win32com_, "logger", None)
