...
def TestExplorerEvents():
    global bVisibleEventFired
    try:
        iexplore = win32com_.client.DispatchWithEvents(
    ...
    bVisibleEventFired = 0
    ie = win32com_.client.Dispatch("InternetExplorer.Application")
    ie_events = win32com_.client.DispatchWithEvents(ie, ExplorerEvents)
    ie.Visible = 1
    if not bVisibleEventFired:
        raise RuntimeError("The IE event did not appear to fire!")
    ie.Quit()
...
def TestAll():
    try:
        try:
            try:
                iexplore = win32com_.client.dynamic.Dispatch(
            ...
            iexplore = win32com_.client.Dispatch("InternetExplorer.Application")
            TestExplorer(iexplore)
        except pythoncom.com_error as exc:
