#
# Initialization for the win32com_ package
#
...
#  COM extensions, outside of the win32com_ directory.  These modules, however,
#  look identical to win32com_ built-in modules.
#  This is the technique that we use for the "standard" COM extensions.
#  eg "win32com_.mapi" or "win32com_.axscript" both work, even though they do not
#  live under the main win32com_ directory.
__gen_path__ = ""
__build_path__ = None
...
# (which the win32com_ developers do!)
...
# normal module and use that (ie, "win32com_.gen_py" may already exist as
# a package.
if not __gen_path__:
    try:
        ...
        # TODO: Not working in PyJL
        # __gen_path__ = next(iter(sys.modules["win32com_.gen_py"].__path__))
    except ImportError:
        # If a win32com_\gen_py directory already exists, then we use it
        ...
        if not os.path.isdir(__gen_path__):
            # We used to dynamically create a directory under win32com_ -
...
# set that up.
if "win32com_.gen_py" not in sys.modules:
    # Create a "win32com_.gen_py", but with a custom __path__
    import types
