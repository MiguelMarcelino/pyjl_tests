...
#
from win32com_.server.exception import IsCOMServerException
from win32com_.util import IIDToInterfaceName
import win32com_
...
class DispatcherBase:
    ...
    def __init__(self, policyClass, object):
        ...
        self.logger = getattr(win32com_, "logger", None)
...
class DispatcherWin32dbg(DispatcherBase):
    ...
    def _HandleException_(self):
        ...
        # Save details away.
        typ, val, tb = exc_info()
        # import win32com_.gen_py.debugger, win32com_.gen_py.debugger.dbgcon
        debug = 0
        try:
            raise typ(val)
            ...
            debug = win32com_.gen_py.debugger.GetDebugger().get_option(
        ...
        if debug:
            try:
                win32com_.gen_py.debugger.post_mortem(tb, typ, val)  # The original exception
            except:
                traceback.print_exc()
