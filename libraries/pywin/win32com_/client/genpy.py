...
class DispatchItem(build.DispatchItem, WritableItem):
    ...
    def WriteEventSinkClassHeader(self, generator):
        ...
        print("\t\t\timport win32com_.server.util", file=stream)
        print(
            "\t\t\tfrom win32com_.server.policy import EventHandlerPolicy", file=stream
        )
        print(
        ...
        print("\t\timport win32com_.server.util", file=stream)
        print(
            "\t\tif iid==self.CLSID_Sink: return win32com_.server.util.wrap(self)",
            file=stream,
        )
        print(file=stream)
        self.bWritten = 1
    ...
    def WriteClassBody(self, generator):
        ...
        print(
            "\t\treturn win32com_.client.util.Iterator(ob, %s)" % resultCLSID,
            file=stream,
        )
...
class Generator:
    ...
    def do_gen_file_header(self):
        ...
        print(
            "import win32com_.client.CLSIDToClass, pythoncom, pywintypes", file=self.file
        )
        print("import win32com_.client.util", file=self.file)
        print("from pywintypes import IID", file=self.file)
        print("from win32com_.client import Dispatch", file=self.file)
        print(file=self.file)
        print(
    ...
    def do_generate(self):
        ...
        if enumItems:
            print(
                "win32com_.client.constants.__dicts__.append(constants.__dict__)",
                file=stream,
            )
        print(file=stream)
    ...
    def generate_child(self, child, dir):
        ...
        minor = la[4]
        self.base_mod_name = (
            "win32com_.gen_py." + str(clsid)[1:-1] + "x%sx%sx%s" % (lcid, major, minor)
        )
        try:
    ...
    def checkWriteDispatchBaseClass(self):
        if not self.bHaveWrittenDispatchBaseClass:
            print("from win32com_.client import DispatchBaseClass", file=self.file)
            self.bHaveWrittenDispatchBaseClass = 1

    def checkWriteCoClassBaseClass(self):
        if not self.bHaveWrittenCoClassBaseClass:
            print("from win32com_.client import CoClassBaseClass", file=self.file)
