...
# Changed
import win32com_, win32com_.client
...
is_readonly = is_zip = hasattr(win32com_, "__loader__") and hasattr(
    win32com_.__loader__, "archive"
)
...
def _LoadDicts():
    ...
    if is_zip:
        ...
        dicts_path = os.path.join(win32com_.__gen_path__, "dicts.dat")
        ...
        f = open(os.path.join(win32com_.__gen_path__, "dicts.dat"), "rb")
    try:
...
def GetGeneratePath():
    ...
    try:
        ...
        # os.mkdir(win32com_.__gen_path__)
    except os.error:
        pass
    try:
        fname = os.path.join(win32com_.__gen_path__, "__init__.py")
        os.stat(fname)
    except os.error:
        f = open(fname, "w")
        f.write(
...
#
# The helpers for win32com_.client.Dispatch and OCX clients.
#
...
def GetGeneratedInfos():
    zip_pos = win32com_.__gen_path__.find(".zip\\")
    if zip_pos >= 0:
        import zipfile

        zip_file = win32com_.__gen_path__[: zip_pos + 4]
        zip_path = win32com_.__gen_path__[zip_pos + 5 :].replace("\\", "/")
...
def _GetModule(fname):
    ...
    mod_name = "win32com_.gen_py.%s" % fname # TODO: Once fixed, change to: win32com_.gen_py.%s
...
def _Dump():
    print("Cache is in directory", win32com_.__gen_path__)
    # Build a unique dir
    d = {}
