...
mutable struct MapEntry <: AbstractMapEntry
    ...
    MapEntry(
        ...
        new(desc_or_id, names, doc, resultCLSID, resultDoc, hidden, wasProperty)
    end
end
...
mutable struct OleItem <: AbstractOleItem
    ...
    OleItem(
        ...
        if doc
            python_name = MakePublicAttributeName(doc[1])
        else
            python_name = nothing
        end
        new(doc, bWritten, bIsDispatch, bIsSink, clsid, co_class)
    end
end
...
function MakeDispatchFuncMethod(
    ...
    if length(bad_params) == 0 && length(retDesc) == 2 && retDesc[2] == 0
        ...
        if rd ∈ NoTranslateMap
            s = "$(linePrefix)\treturn self._oleobj_.InvokeTypes($(id), LCID, $(fdesc[5]), $(retDesc), $(argsDesc)$(_BuildArgList(fdesc, names)))"
        elseif rd ∈ [pythoncom.VT_DISPATCH, pythoncom.VT_UNKNOWN]
            s = "$(linePrefix)\tret = self._oleobj_.InvokeTypes($(id), LCID, $(fdesc[5]), $(retDesc), $(repr(argsDesc))$(_BuildArgList(fdesc, names)))\n"
            ...
            s = s + "$(linePrefix)\t\tret = Dispatch(ret, $(repr(name)), $(resclsid))\n"
            s = s + "$(linePrefix)\treturn ret"
        elseif rd == pythoncom.VT_BSTR
            s = "$(linePrefix)\t# Result is a Unicode object\n"
            s =
                s +
                "$(linePrefix)\treturn self._oleobj_.InvokeTypes($(id), LCID, $(fdesc[5]), $(retDesc), $(repr(argsDesc))$(_BuildArgList(fdesc, names)))"
        end
    end
    if s === nothing
        s = "$(linePrefix)\treturn self._ApplyTypes_($(id), $(fdesc[5]), $(retDesc), $(argsDesc), $(repr(name)), $(resclsid)$(_BuildArgList(fdesc, names)))"
    end
    push!(ret, s)
    push!(ret, "")
    return ret
...
function MakeVarArgsFuncMethod(
    ...
    push!(ret, s * string(entry.dispid) + ",0,$(invoketype),1)+args)),\'$(names[1])\')")
    push!(ret, "")
    return ret
end
...
mutable struct LazyDispatchItem <: AbstractLazyDispatchItem
    ...
    LazyDispatchItem(attr, doc, clsid = attr[1]) = begin
