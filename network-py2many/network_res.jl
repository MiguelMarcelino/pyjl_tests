...
function update_mini_batch(self::AbstractNetwork, mini_batch, eta)
    ...
    for (x, y) in mini_batch
        # Temporary
        x, y = array_or_arrays(x), array_or_arrays(y)
...
function backprop(self::AbstractNetwork, x, y)::Tuple
    ...
    nabla_b = [array_or_arrays(zeros(Float64, size(b))) for b in self.biases]
    nabla_w = [array_or_arrays(zeros(Float64, size(w))) for w in self.weights]
    activation = x
    activations = [x]
    zs = []
    for (b, w) in zip(self.biases, self.weights)
        z = (w * activation) .+ b # removed broadcast
        # Conversion
        push!(zs, array_or_arrays(z))
        activation = sigmoid(z)
        push!(activations, array_or_arrays(activation))
    end

    delta = cost_derivative(self, activations[end], y) * sigmoid_prime.(zs[end])
    print(typeof(nabla_b))
    nabla_b[end] = delta
