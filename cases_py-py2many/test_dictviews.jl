...
@oodef mutable struct CustomSet <: set
                    
                    
                    
                end
                function intersection(self::@like(CustomSet), other)::CustomSet
...
@oodef mutable struct DictSetTest <: unittest.TestCase
                    
                    
                    
                end
                function test_constructors_not_callable(self::@like(DictSetTest))
kt = type_(keys(Dict()))
@test_throws
@test_throws
...
function test_dict_keys(self::@like(DictSetTest))
d = Dict(1 => 10, "a" => "ABC")
keys_ = keys_(d)
@test (length(keys_) == 2)
@test (Set(keys_) == Set([1, "a"]))
@test (keys_ == Set([1, "a"]))
@test (keys_ != Set([1, "a", "b"]))
@test (keys_ != Set([1, "b"]))
@test (keys_ != Set([1]))
@test (keys_ != 42)
assertIn(self, 1, keys_)
assertIn(self, "a", keys_)
assertNotIn(self, 10, keys_)
assertNotIn(self, "Z", keys_)
@test (keys_(d) == keys_(d))
e = Dict(1 => 11, "a" => "def")
@test (keys_(d) == keys_(e))
# Delete Unsupported
# del(e)
@test (keys_(d) != keys_(e))
end

function test_dict_items(self::@like(DictSetTest))
d = Dict(1 => 10, "a" => "ABC")
items = collect(d)
@test (length(items) == 2)
@test (Set(items) == Set([(1, 10), ("a", "ABC")]))
@test (items == Set([(1, 10), ("a", "ABC")]))
@test (items != Set([(1, 10), ("a", "ABC"), "junk"]))
@test (items != Set([(1, 10), ("a", "def")]))
@test (items != Set([(1, 10)]))
@test (items != 42)
...
function test_dict_mixed_keys_items(self::@like(DictSetTest))
d = Dict{Tuple{Int64}, Int64}((1, 1) => 11, (2, 2) => 22)
e = Dict{Int64, Int64}(1 => 1, 2 => 2)
@test (keys(d) == collect(e))
@test (collect(d) != keys(e))
end

function test_dict_values(self::@like(DictSetTest))
d = Dict(1 => 10, "a" => "ABC")
values_ = values_(d)
@test (Set(values_) == Set([10, "ABC"]))
@test (length(values_) == 2)
end

function test_dict_repr(self::@like(DictSetTest))
d = Dict(1 => 10, "a" => "ABC")
...
function test_keys_set_operations(self::@like(DictSetTest))
d1 = Dict{String, Int64}("a" => 1, "b" => 2)
d2 = Dict{String, Int64}("b" => 3, "c" => 2)
d3 = Dict{String, Int64}("d" => 4, "e" => 5)
d4 = Dict{String, Int64}("d" => 4)
@test (keys(d1) & keys(d1) == Set(["a", "b"]))
@test (keys(d1) & keys(d2) == Set(["b"]))
@test (keys(d1) & keys(d3) == Set())
@test (keys(d1) & Set(keys(d1)) == Set(["a", "b"]))
@test (keys(d1) & Set(keys(d2)) == Set(["b"]))
@test (keys(d1) & Set(keys(d3)) == Set())
@test (keys(d1) & tuple(keys(d1)) == Set(["a", "b"]))
@test (keys(d3) & keys(d4) == Set(["d"]))
@test (keys(d4) & keys(d3) == Set(["d"]))
@test (keys(d4) & Set(keys(d3)) == Set(["d"]))
@test isa(self, keys(d4) & frozenset(keys(d3)))
@test isa(self, frozenset(keys(d3)) & keys(d4))
@test self === type_(keys(d4) & CustomSet(keys(d3)))
@test self === type_(keys(d1) & [])
@test self === type_([] & keys(d1))
@test (keys(d1) | keys(d1) == Set(["a", "b"]))
@test (keys(d1) | keys(d2) == Set(["a", "b", "c"]))
@test (keys(d1) | keys(d3) == Set(["a", "b", "d", "e"]))
@test (keys(d1) | Set(keys(d1)) == Set(["a", "b"]))
@test (keys(d1) | Set(keys(d2)) == Set(["a", "b", "c"]))
@test (keys(d1) | Set(keys(d3)) == Set(["a", "b", "d", "e"]))
@test (keys(d1) | (1, 2) == Set(["a", "b", 1, 2]))
@test (keys(d1) ⊻ keys(d1) == Set())
@test (keys(d1) ⊻ keys(d2) == Set(["a", "c"]))
@test (keys(d1) ⊻ keys(d3) == Set(["a", "b", "d", "e"]))
@test (keys(d1) ⊻ Set(keys(d1)) == Set())
@test (keys(d1) ⊻ Set(keys(d2)) == Set(["a", "c"]))
@test (keys(d1) ⊻ Set(keys(d3)) == Set(["a", "b", "d", "e"]))
@test (keys(d1) ⊻ tuple(keys(d2)) == Set(["a", "c"]))
@test (keys(d1) - keys(d1) == Set())
@test (keys(d1) - keys(d2) == Set(["a"]))
@test (keys(d1) - keys(d3) == Set(["a", "b"]))
@test (keys(d1) - Set(keys(d1)) == Set())
@test (keys(d1) - Set(keys(d2)) == Set(["a"]))
@test (keys(d1) - Set(keys(d3)) == Set(["a", "b"]))
@test (keys(d1) - (0, 1) == Set(["a", "b"]))
@test !(isdisjoint(keys(d1), keys(d1)))
@test !(isdisjoint(keys(d1), keys(d2)))
@test !(isdisjoint(keys(d1), collect(keys(d2))))
@test !(isdisjoint(keys(d1), Set(keys(d2))))
@test isdisjoint(keys(d1), Set(["x", "y", "z"]))
@test isdisjoint(keys(d1), ["x", "y", "z"])
@test isdisjoint(keys(d1), Set(["x", "y", "z"]))
@test isdisjoint(keys(d1), Set(["x", "y"]))
@test isdisjoint(keys(d1), ["x", "y"])
@test isdisjoint(keys(d1), Dict())
@test isdisjoint(keys(d1), keys(d3))
de = Dict()
@test isdisjoint(keys(de), Set())
@test isdisjoint(keys(de), [])
@test isdisjoint(keys(de), keys(de))
@test isdisjoint(keys(de), [1])
end

function test_items_set_operations(self::@like(DictSetTest))
d1 = Dict{String, Int64}("a" => 1, "b" => 2)
d2 = Dict{String, Int64}("a" => 2, "b" => 2)
d3 = Dict{String, Int64}("d" => 4, "e" => 5)
...
function test_set_operations_with_iterator(self::@like(DictSetTest))
origin = Dict{Int64, Int64}(1 => 2, 3 => 4)
...
function test_set_operations_with_noniterable(self::@like(DictSetTest))
...
function test_recursive_repr(self::@like(DictSetTest))
d = Dict()
d[42] = values(d)
r = repr(d)
@test isa(self, r)
...
function test_deeply_nested_repr(self::@like(DictSetTest))
d = Dict()
for i in 0:sys.getrecursionlimit() + 99
d = Dict{Int64, Any}(42 => values(d))
end
@test_throws
end

function test_copy(self::@like(DictSetTest))
d = Dict(1 => 10, "a" => "ABC")
@test_throws
@test_throws
@test_throws
...
function test_compare_error(self::@like(DictSetTest))
(k1, k2) = (BadEq(), BadEq())
(v1, v2) = (BadEq(), BadEq())
d = Dict(k1 => v1)
assertIn(self, k1, d)
...
function test_pickle(self::@like(DictSetTest))
d = Dict(1 => 10, "a" => "ABC")
...
function test_abc_registry(self::@like(DictSetTest))
d = dict(a = 1)
