...
abstract type AbstractCustomSet <: set end
...
@test (collect(d) == collect(d))
e = copy(d)
@test (collect(d) == items(e))
e["a"] = "def"
...
d2 = Dict("a" => 2, "b" => 2)
d3 = Dict("d" => 4, "e" => 5)
@test (collect(d1) & collect(d1) == Set([("a", 1), ("b", 2)]))
@test (collect(d1) & collect(d2) == Set([("b", 2)]))
@test (collect(d1) & collect(d3) == set())
@test (collect(d1) & set(collect(d1)) == Set([("a", 1), ("b", 2)]))
@test (collect(d1) & set(collect(d2)) == Set([("b", 2)]))
@test (collect(d1) & set(collect(d3)) == set())
@test (collect(d1) | collect(d1) == Set([("a", 1), ("b", 2)]))
@test (collect(d1) | collect(d2) == Set([("a", 1), ("a", 2), ("b", 2)]))
@test (collect(d1) | collect(d3) == Set([("a", 1), ("b", 2), ("d", 4), ("e", 5)]))
@test (collect(d1) | set(collect(d1)) == Set([("a", 1), ("b", 2)]))
@test (collect(d1) | set(collect(d2)) == Set([("a", 1), ("a", 2), ("b", 2)]))
@test (collect(d1) | set(collect(d3)) == Set([("a", 1), ("b", 2), ("d", 4), ("e", 5)]))
@test (collect(d1)  ⊻  collect(d1) == set())
@test (collect(d1)  ⊻  collect(d2) == Set([("a", 1), ("a", 2)]))
@test (collect(d1)  ⊻  collect(d3) == Set([("a", 1), ("b", 2), ("d", 4), ("e", 5)]))
@test (collect(d1) - collect(d1) == set())
@test (collect(d1) - collect(d2) == Set([("a", 1)]))
@test (collect(d1) - collect(d3) == Set([("a", 1), ("b", 2)]))
@test (collect(d1) - set(collect(d1)) == set())
@test (collect(d1) - set(collect(d2)) == Set([("a", 1)]))
@test (collect(d1) - set(collect(d3)) == Set([("a", 1), ("b", 2)]))
@test !(isdisjoint(collect(d1), collect(d1)))
@test !(isdisjoint(collect(d1), collect(d2)))
@test !(isdisjoint(collect(d1), collect(collect(d2))))
@test !(isdisjoint(collect(d1), set(collect(d2))))
@test isdisjoint(collect(d1), Set(["x", "y", "z"]))
@test isdisjoint(collect(d1), ["x", "y", "z"])
@test isdisjoint(collect(d1), set(["x", "y", "z"]))
@test isdisjoint(collect(d1), set(["x", "y"]))
@test isdisjoint(collect(d1), Dict())
@test isdisjoint(collect(d1), collect(d3))
de = Dict()
@test isdisjoint(collect(de), set())
@test isdisjoint(collect(de), [])
@test isdisjoint(collect(de), collect(de))
...
assertIn(self, (k1, v1), collect(d))
assertRaises(self, Exc, d.__contains__, k2)
assertRaises(self, Exc, keys(d).__contains__, k2)
assertRaises(self, Exc, collect(d).__contains__, (k2, v1))
assertRaises(self, Exc, collect(d).__contains__, (k1, v2))
...
@test_throws (TypeError, pickle.PicklingError) pickle.dumps(collect(d), proto)
end
end
