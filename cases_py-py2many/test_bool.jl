...
function test_operator(self::BoolTest)
assertIs(self, operator.truth(0), false)
assertIs(self, operator.truth(1), true)
assertIs(self, operator.not_(1), false)
assertIs(self, operator.not_(0), true)
assertIs(self, operator.contains([], 1), false)
assertIs(self, operator.contains([1], 1), true)
assertIs(self, operator.lt(0, 0), false)
assertIs(self, operator.lt(0, 1), true)
assertIs(self, operator.is_(true, true), true)
assertIs(self, operator.is_(true, false), false)
assertIs(self, operator.is_not(true, true), false)
assertIs(self, operator.is_not(true, false), true)
end

function test_marshal(self::BoolTest)
assertIs(self, marshal.loads(marshal.dumps(true)), true)
assertIs(self, marshal.loads(marshal.dumps(false)), false)
end

function test_pickle(self::BoolTest)
for proto in 0:pickle.HIGHEST_PROTOCOL
assertIs(self, pickle.loads(pickle.dumps(true, proto)), true)
assertIs(self, pickle.loads(pickle.dumps(false, proto)), false)
end
end

function test_picklevalues(self::BoolTest)
@test (pickle.dumps(true, 0) == b"I01\n.")
@test (pickle.dumps(false, 0) == b"I00\n.")
@test (pickle.dumps(true, 1) == b"I01\n.")
@test (pickle.dumps(false, 1) == b"I00\n.")
@test (pickle.dumps(true, 2) == b"\x80\x02\x88.")
@test (pickle.dumps(false, 2) == b"\x80\x02\x89.")
end
