...
@oodef mutable struct AlwaysEqual
                    
                    
                    
                end
                function __eq__(self::@like(AlwaysEqual), other)::Bool
return true
end


@oodef mutable struct DictVersionTests <: unittest.TestCase
                    
                    dict
seen_versions
type2test
                    
function new(dict = nothing, seen_versions = Set(), type2test = dict)
dict = dict
seen_versions = seen_versions
type2test = type2test
new(dict, seen_versions, type2test)
end

                end
                function setUp(self::@like(DictVersionTests))
self.seen_versions = Set()
self.dict = nothing
end

function check_version_unique(self::@like(DictVersionTests), mydict)
version = dict_get_version(_testcapi, mydict)
assertNotIn(self, version, self.seen_versions)
add(self.seen_versions, version)
end

function check_version_changed(self::@like(DictVersionTests), mydict, method, args...)
result = method(args..., None = kw)
check_version_unique(self, mydict)
return result
end

function check_version_dont_change(self::@like(DictVersionTests), mydict, method, args...)
...
function new_dict(self::@like(DictVersionTests), args...)
d = type2test(self, args..., None = kw)
check_version_unique(self, d)
return d
end

function test_constructor(self::@like(DictVersionTests))
empty1 = new_dict(self)
empty2 = new_dict(self)
empty3 = new_dict(self)
nonempty1 = new_dict(self, x = "x")
nonempty2 = new_dict(self, x = "x", y = "y")
end

function test_copy(self::@like(DictVersionTests))
d = new_dict(self, a = 1, b = 2)
d2 = check_version_dont_change(self, d, d.copy)
check_version_unique(self, d2)
end

function test_setitem(self::@like(DictVersionTests))
d = new_dict(self)
...
function test_setitem_same_value(self::@like(DictVersionTests))
value = object()
d = new_dict(self)
check_version_changed(self, d, d.__setitem__)
check_version_dont_change(self, d, d.__setitem__)
check_version_dont_change(self, d, d.update, key = value)
d2 = new_dict(self, key = value)
check_version_dont_change(self, d, d.update)
end

function test_setitem_equal(self::@like(DictVersionTests))
value1 = AlwaysEqual()
value2 = AlwaysEqual()
@test value1 == value2
@test !(value1 != value2)
...
check_version_changed(self, d, d.__setitem__)
@test self === d["key"]
check_version_changed(self, d, d.update, key = value1)
@test self === d["key"]
d2 = new_dict(self, key = value2)
check_version_changed(self, d, d.update)
@test self === d["key"]
end

function test_setdefault(self::@like(DictVersionTests))
d = new_dict(self)
check_version_changed(self, d, d.setdefault)
check_version_dont_change(self, d, d.setdefault)
end

function test_delitem(self::@like(DictVersionTests))
d = new_dict(self, key = "value")
check_version_changed(self, d, d.__delitem__)
check_version_dont_change(self, d, @test_throws)
end

function test_pop(self::@like(DictVersionTests))
d = new_dict(self, key = "value")
check_version_changed(self, d, d.pop)
check_version_dont_change(self, d, @test_throws)
end

function test_popitem(self::@like(DictVersionTests))
d = new_dict(self, key = "value")
check_version_changed(self, d, d.popitem)
check_version_dont_change(self, d, @test_throws)
end

function test_update(self::@like(DictVersionTests))
d = new_dict(self, key = "value")
check_version_dont_change(self, d, d.update)
check_version_changed(self, d, d.update, key = "new value")
d2 = new_dict(self, key = "value 3")
check_version_changed(self, d, d.update)
end

function test_clear(self::@like(DictVersionTests))
d = new_dict(self, key = "value")
...
@oodef mutable struct DictSubtypeVersionTests <: DictVersionTests
                    
                    dict
seen_versions
type2test::AbstractDict
                    
function new(dict = nothing, seen_versions = Set(), type2test::Dict = Dict)
dict = dict
seen_versions = seen_versions
type2test = type2test
new(dict, seen_versions, type2test)
