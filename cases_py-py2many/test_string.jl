...
@oodef mutable struct AnyAttr
                    
                    
                    
                end
                function Base.getproperty(self::@like(AnyAttr), attr::Symbol)
...
@oodef mutable struct NamespaceFormatter <: string.Formatter
                    
                    namespace
                    
function new(namespace = Dict())
@mk begin
string()
namespace = namespace
end
                ...
                function get_value(self::@like(NamespaceFormatter), key, args, kwds)
if isa(key, String)
try
return kwds[key + 1]
catch exn
...
string.get_value(key, args, kwds)
end
end


@oodef mutable struct CallFormatter <: string.Formatter
                    
                    
                    
                end
                function format_field(self::@like(CallFormatter), value, format_spec)
return value()
end


@oodef mutable struct XFormatter <: string.Formatter
                    
                    
                    
                end
                function convert_field(self::@like(XFormatter), value, conversion)
if conversion == "x"
return nothing
end
return string.Formatter(value, conversion)
end


@oodef mutable struct BarFormatter <: string.Formatter
                    
                    
                    
                end
                @resumable function parse(self::@like(BarFormatter), format_string)
for field in split(format_string, "|")
if field[1] == "+"
(field_name, _, format_spec) = partition(field[2:end], ":")
@yield ("", field_name, format_spec, nothing)
else
@yield (field, nothing, nothing, nothing)
end
end
end


@oodef mutable struct CheckAllUsedFormatter <: string.Formatter
                    
                    
                    
                end
                function check_unused_args(self::@like(CheckAllUsedFormatter), used_args, args, kwargs)
...
@oodef mutable struct ModuleTest <: unittest.TestCase
                    
                    
                    
                end
                function test_attrs(self::@like(ModuleTest))
...
function test_capwords(self::@like(ModuleTest))
@test (string.capwords("abc def ghi") == "Abc Def Ghi")
@test (string.capwords("abc\tdef\nghi") == "Abc Def Ghi")
@test (string.capwords("abc\t   def  \nghi") == "Abc Def Ghi")
@test (string.capwords("ABC DEF GHI") == "Abc Def Ghi")
@test (string.capwords("ABC-DEF-GHI", "-") == "Abc-Def-Ghi")
@test (string.capwords("ABC-def DEF-ghi GHI") == "Abc-def Def-ghi Ghi")
@test (string.capwords("   aBc  DeF   ") == "Abc Def")
@test (string.capwords("\taBc\tDeF\t") == "Abc Def")
@test (string.capwords("\taBc\tDeF\t", "\t") == "\tAbc\tDef\t")
end

function test_basic_formatter(self::@like(ModuleTest))
fmt = string.Formatter()
@test (format(fmt, "foo") == "foo")
@test (format(fmt, "foo{0}", "bar") == "foobar")
@test (format(fmt, "foo{1}{0}-{1}", "bar", 6) == "foo6bar-6")
@test_throws
@test_throws
end

function test_format_keyword_arguments(self::@like(ModuleTest))
fmt = string.Formatter()
@test (format(fmt, "-{arg}-", arg = "test") == "-test-")
@test_throws
@test (format(fmt, "-{self}-", self = "test") == "-test-")
@test_throws
@test (format(fmt, "-{format_string}-", format_string = "test") == "-test-")
@test_throws
assertRaisesRegex(self, TypeError, "format_string") do 
format(fmt, format_string = "-{arg}-", arg = "test")
end
end

function test_auto_numbering(self::@like(ModuleTest))
fmt = string.Formatter()
@test (format(fmt, "foo{}{}", "bar", 6) == "foo$("bar")$(6)")
@test (format(fmt, "foo{1}{num}{1}", nothing, "bar", num = 6) == "foo1$("bar")1")
@test (format(fmt, "{:^{}}", "bar", 6) == ":^$(6)")
@test (format(fmt, "{:^{}} {}", "bar", 6, "X") == ":^$(6) $("X")")
@test (format(fmt, "{:^{pad}}{}", "foo", "bar", pad = 6) == format("{:^{pad}}{}", "foo", "bar", pad = 6))
@test_throws ValueError do 
format(fmt, "foo{1}{}", "bar", 6)
end
@test_throws ValueError do 
format(fmt, "foo{}{1}", "bar", 6)
end
...
function test_conversion_specifiers(self::@like(ModuleTest))
fmt = string.Formatter()
@test (format(fmt, "-{arg!r}-", arg = "test") == "-\'test\'-")
@test (format(fmt, "{0!s}", "test") == "test")
@test_throws
@test (format(fmt, "{0!a}", 42) == "42")
@test (format(fmt, "{0!a}", string.ascii_letters) == "\'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ\'")
@test (format(fmt, "{0!a}", Char(255)) == "\'\\xff\'")
@test (format(fmt, "{0!a}", Char(256)) == "\'\\u0100\'")
end

function test_name_lookup(self::@like(ModuleTest))
fmt = string.Formatter()
x = AnyAttr()
@test (format(fmt, "{0.lumber}{0.jack}", x) == "lumberjack")
@test_throws AttributeError do 
format(fmt, "{0.lumber}{0.jack}", "")
end
end

function test_index_lookup(self::@like(ModuleTest))
fmt = string.Formatter()
lookup = ["eggs", "and", "spam"]
@test (format(fmt, "{0[2]}{0[0]}", lookup) == "spameggs")
@test_throws IndexError do 
format(fmt, "{0[2]}{0[0]}", [])
end
@test_throws KeyError do 
format(fmt, "{0[2]}{0[0]}", Dict())
end
end

function test_override_get_value(self::@like(ModuleTest))
fmt = NamespaceFormatter(Dict{str, str}("greeting" => "hello"))
@test (format(fmt, "{greeting}, world!") == "hello, world!")
end

function test_override_format_field(self::@like(ModuleTest))
fmt = CallFormatter()
@test (format(fmt, "*{0}*", () -> "result") == "*result*")
end

function test_override_convert_field(self::@like(ModuleTest))
fmt = XFormatter()
@test (format(fmt, "{0!r}:{0!x}", "foo", "foo") == "\'foo\':None")
end

@resumable function test_override_parse(self::@like(ModuleTest))
fmt = BarFormatter()
@test (format(fmt, "*|+0:^10s|*", "foo") == "*   foo    *")
end

function test_check_unused_args(self::@like(ModuleTest))
fmt = CheckAllUsedFormatter()
@test (format(fmt, "{0}", 10) == "10")
@test (format(fmt, "{0}{i}", 10, i = 100) == "10100")
@test (format(fmt, "{0}{i}{1}", 10, 20, i = 100) == "1010020")
@test_throws
@test_throws
@test_throws
@test_throws
...
function test_vformat_recursion_limit(self::@like(ModuleTest))
fmt = string.Formatter()
args = ()
kwargs = dict(i = 100)
@test_throws ValueError do err 
...
@oodef mutable struct Mapping
                    
                    
                    
                end
                function __getitem__(self::@like(Mapping), name)
obj = self
...
@oodef mutable struct PathPattern <: Template
                    
                    idpattern::String
                    
function new(idpattern::String = "[_a-z][._a-z0-9]*")
idpattern = idpattern
new(idpattern)
end
...
@oodef mutable struct MyPattern <: Template
                    
                    flags::Int64
                    
function new(flags::Int64 = 0)
flags = flags
new(flags)
end
...
function new(braceidpattern::String = "[A-Z]+", flags::Int64 = 0, idpattern::String = "[a-z]+")
braceidpattern = braceidpattern
flags = flags
idpattern = idpattern
...
@oodef mutable struct BadPattern <: Template
...
@oodef mutable struct MyTemplate <: Template
...
@oodef mutable struct AmpersandTemplate <: Template
                    
                    delimiter::String
                    
function new(delimiter::String = "&")
delimiter = delimiter
new(delimiter)
end
...
@oodef mutable struct PieDelims <: Template
...
@oodef mutable struct TestTemplate <: unittest.TestCase
                    
                    
                    
                end
                function test_regular_templates(self::@like(TestTemplate))
s = Template("\$who likes to eat a bag of \$what worth \$\$100")
@test (substitute(s, dict(who = "tim", what = "ham")) == "tim likes to eat a bag of ham worth \$100")
@test_throws
@test_throws
end

function test_regular_templates_with_braces(self::@like(TestTemplate))
...
function test_regular_templates_with_upper_case(self::@like(TestTemplate))
s = Template("\$WHO likes \${WHAT} for \${MEAL}")
d = dict(WHO = "tim", WHAT = "ham", MEAL = "dinner")
@test (substitute(s, d) == "tim likes ham for dinner")
end

function test_regular_templates_with_non_letters(self::@like(TestTemplate))
s = Template("\$_wh0_ likes \${_w_h_a_t_} for \${mea1}")
d = dict(_wh0_ = "tim", _w_h_a_t_ = "ham", mea1 = "dinner")
@test (substitute(s, d) == "tim likes ham for dinner")
end

function test_escapes(self::@like(TestTemplate))
eq = self.assertEqual
s = Template("\$who likes to eat a bag of \$\$what worth \$\$100")
eq(substitute(s, dict(who = "tim", what = "ham")), "tim likes to eat a bag of \$what worth \$100")
s = Template("\$who likes \$\$")
eq(substitute(s, dict(who = "tim", what = "ham")), "tim likes \$")
end

function test_percents(self::@like(TestTemplate))
eq = self.assertEqual
...
function test_stringification(self::@like(TestTemplate))
eq = self.assertEqual
...
function test_tupleargs(self::@like(TestTemplate))
eq = self.assertEqual
...
function test_SafeTemplate(self::@like(TestTemplate))
eq = self.assertEqual
s = Template("\$who likes \${what} for \${meal}")
eq(safe_substitute(s, dict(who = "tim")), "tim likes \${what} for \${meal}")
eq(safe_substitute(s, dict(what = "ham")), "\$who likes ham for \${meal}")
eq(safe_substitute(s, dict(what = "ham", meal = "dinner")), "\$who likes ham for dinner")
eq(safe_substitute(s, dict(who = "tim", what = "ham")), "tim likes ham for \${meal}")
eq(safe_substitute(s, dict(who = "tim", what = "ham", meal = "dinner")), "tim likes ham for dinner")
end

function test_invalid_placeholders(self::@like(TestTemplate))
raises = @test_throws
s = Template("\$who likes \$")
...
function test_idpattern_override(self::@like(TestTemplate))
m = Mapping()
m.bag = Bag()
m.bag.foo = Bag()
m.bag.foo.who = "tim"
...
@test (substitute(s, m) == "tim likes to eat a bag of ham")
end

function test_flags_override(self::@like(TestTemplate))
s = MyPattern("\$wHO likes \${WHAT} for \${meal}")
d = dict(wHO = "tim", WHAT = "ham", meal = "dinner", w = "fred")
@test_throws
@test (safe_substitute(s, d) == "fredHO likes \${WHAT} for dinner")
end

function test_idpattern_override_inside_outside(self::@like(TestTemplate))
m = dict(foo = "foo", BAR = "BAR")
s = MyPattern("\$foo \${BAR}")
@test (substitute(s, m) == "foo BAR")
end

function test_idpattern_override_inside_outside_invalid_unbraced(self::@like(TestTemplate))
m = dict(foo = "foo", BAR = "BAR")
s = MyPattern("\$FOO")
@test_throws
s = MyPattern("\${bar}")
...
function test_pattern_override(self::@like(TestTemplate))
m = Mapping()
m.bag = Bag()
m.bag.foo = Bag()
m.bag.foo.who = "tim"
...
function test_braced_override(self::@like(TestTemplate))
tmpl = "PyCon in \$@@location@@"
t = MyTemplate(tmpl)
@test_throws
val = substitute(t, Dict{str, str}("location" => "Cleveland"))
@test (val == "PyCon in Cleveland")
end

function test_braced_override_safe(self::@like(TestTemplate))
tmpl = "PyCon in \$@@location@@"
t = MyTemplate(tmpl)
@test (safe_substitute(t) == tmpl)
val = safe_substitute(t, Dict{str, str}("location" => "Cleveland"))
@test (val == "PyCon in Cleveland")
end

function test_invalid_with_no_lines(self::@like(TestTemplate))
s = MyTemplate("")
@test_throws ValueError do err 
substitute(s, Dict())
end
...
function test_unicode_values(self::@like(TestTemplate))
s = Template("\$who likes \$what")
d = dict(who = "t\xffm", what = "fþ\fed")
@test (substitute(s, d) == "t\xffm likes fþ\fed")
end

function test_keyword_arguments(self::@like(TestTemplate))
eq = self.assertEqual
s = Template("\$who likes \$what")
eq(substitute(s, who = "tim", what = "ham"), "tim likes ham")
eq(substitute(s, dict(who = "tim"), what = "ham"), "tim likes ham")
eq(substitute(s, dict(who = "fred", what = "kung pao"), who = "tim", what = "ham"), "tim likes ham")
s = Template("the mapping is \$mapping")
eq(substitute(s, dict(foo = "none"), mapping = "bozo"), "the mapping is bozo")
eq(substitute(s, dict(mapping = "one"), mapping = "two"), "the mapping is two")
s = Template("the self is \$self")
eq(substitute(s, self = "bozo"), "the self is bozo")
end

function test_keyword_arguments_safe(self::@like(TestTemplate))
eq = self.assertEqual
raises = @test_throws
s = Template("\$who likes \$what")
eq(safe_substitute(s, who = "tim", what = "ham"), "tim likes ham")
eq(safe_substitute(s, dict(who = "tim"), what = "ham"), "tim likes ham")
eq(safe_substitute(s, dict(who = "fred", what = "kung pao"), who = "tim", what = "ham"), "tim likes ham")
s = Template("the mapping is \$mapping")
eq(safe_substitute(s, dict(foo = "none"), mapping = "bozo"), "the mapping is bozo")
eq(safe_substitute(s, dict(mapping = "one"), mapping = "two"), "the mapping is two")
d = dict(mapping = "one")
raises(TypeError, s.substitute, d, Dict())
raises(TypeError, s.safe_substitute, d, Dict())
s = Template("the self is \$self")
eq(safe_substitute(s, self = "bozo"), "the self is bozo")
end

function test_delimiter_override(self::@like(TestTemplate))
eq = self.assertEqual
raises = @test_throws
s = AmpersandTemplate("this &gift is for &{who} &&")
eq(substitute(s, gift = "bud", who = "you"), "this bud is for you &")
raises(KeyError, s.substitute)
eq(safe_substitute(s, gift = "bud", who = "you"), "this bud is for you &")
