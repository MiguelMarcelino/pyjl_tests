...
function posonly_sum(pos_arg1, arg...)
...
function keywordonly_nodefaults_sum()
return k1 + k2
end
...
function mixedargs_sum(a, b = 0, arg...)
return (((a + b) + k1) + k2) + sum(arg)
end

function mixedargs_sum2(a, b = 0, arg...)
...
k2 = k2
                ...
                function set(self::@like(Foo), p1)
self.k1 = k1
self.k2 = k2
end

function sum(self::@like(Foo))
return self.k1 + self.k2
end


@oodef mutable struct Example
                    
                    
                    
                end
                function f(self::@like(Example))::Tuple
return (k1, k2)
end
...
@oodef mutable struct X
                ...
                function f(self::@like(X))
return __a
end


@oodef mutable struct KeywordOnlyArgTestCase <: unittest.TestCase
                    
                    
                    
                end
                function assertRaisesSyntaxError(self::@like(KeywordOnlyArgTestCase), codestr)
function shouldRaiseSyntaxError(s::@like(KeywordOnlyArgTestCase))
...
function testSyntaxErrorForFunctionDefinition(self::@like(KeywordOnlyArgTestCase))
...
function testSyntaxForManyArguments(self::@like(KeywordOnlyArgTestCase))
fundef = "def f($(join(("i$(i)" for i in 0:299), ", "))):\n  pass\n"
compile(fundef, "<test>", "single")
fundef = "def f(*, $(join(("i$(i)" for i in 0:299), ", "))):\n  pass\n"
compile(fundef, "<test>", "single")
end

function testTooManyPositionalErrorMessage(self::@like(KeywordOnlyArgTestCase))
function f(a::@like(KeywordOnlyArgTestCase), b = nothing)
#= pass =#
end

@test_throws TypeError do exc 
f(1, 2)
end
...
function testSyntaxErrorForFunctionCall(self::@like(KeywordOnlyArgTestCase))
assertRaisesSyntaxError(self, "f(p, k=1, p2)")
assertRaisesSyntaxError(self, "f(p, k1=50, *(1,2), k1=100)")
end

function testRaiseErrorFuncallWithUnexpectedKeywordArgument(self::@like(KeywordOnlyArgTestCase))
@test_throws
@test_throws
@test_throws
try
...
function testFunctionCall(self::@like(KeywordOnlyArgTestCase))
@test (1 == posonly_sum(1))
@test (1 + 2 == posonly_sum(1, None = Dict{str, int}("2" => 2)))
@test ((1 + 2) + 3 == posonly_sum(1))
@test (((1 + 2) + 3) + 4 == posonly_sum(1, None = Dict{str, int}("4" => 4)))
@test (1 == keywordonly_sum(k2 = 1))
@test (1 + 2 == keywordonly_sum(k1 = 1, k2 = 2))
@test (1 + 2 == keywordonly_and_kwarg_sum(k1 = 1, k2 = 2))
@test ((1 + 2) + 3 == keywordonly_and_kwarg_sum(k1 = 1, k2 = 2, k3 = 3))
@test (((1 + 2) + 3) + 4 == keywordonly_and_kwarg_sum(k1 = 1, k2 = 2, None = Dict{str, int}("a" => 3, "b" => 4)))
@test (1 + 2 == mixedargs_sum(1, k1 = 2))
@test ((1 + 2) + 3 == mixedargs_sum(1, 2, k1 = 3))
@test (((1 + 2) + 3) + 4 == mixedargs_sum(1, 2, k1 = 3, k2 = 4))
@test ((((1 + 2) + 3) + 4) + 5 == mixedargs_sum(1, 2, k1 = 4, k2 = 5))
@test (1 + 2 == mixedargs_sum2(1, k1 = 2))
@test ((1 + 2) + 3 == mixedargs_sum2(1, 2, k1 = 3))
@test (((1 + 2) + 3) + 4 == mixedargs_sum2(1, 2, k1 = 3, k2 = 4))
@test ((((1 + 2) + 3) + 4) + 5 == mixedargs_sum2(1, 2, k1 = 4, k2 = 5))
@test (((((1 + 2) + 3) + 4) + 5) + 6 == mixedargs_sum2(1, 2, k1 = 4, k2 = 5, k3 = 6))
@test (((((1 + 2) + 3) + 4) + 5) + 6 == mixedargs_sum2(1, 2, k1 = 4, None = Dict{str, int}("k2" => 5, "k3" => 6)))
@test (1 == sum(Foo(k1 = 1)))
@test (1 + 2 == sum(Foo(k1 = 1, k2 = 2)))
@test ([1, 2, 3] == sortnum(3, 2, 1))
@test ([3, 2, 1] == sortnum(1, 2, 3, reverse = true))
@test (["a", "b", "c"] == sortwords("a", "c", "b"))
@test (["c", "b", "a"] == sortwords("a", "c", "b", reverse = true))
@test (["c", "b", "a"] == sortwords("a", "c", "b", reverse = true, ignore = "ignore"))
end

function testKwDefaults(self::@like(KeywordOnlyArgTestCase))
function foo(p1::@like(KeywordOnlyArgTestCase), p2 = 0)
return ((p1 + p2) + k1) + k2
end

@test (2 == foo.__code__.co_kwonlyargcount)
@test (Dict{str, int}("k2" => 0) == foo.__kwdefaults__)
foo.__kwdefaults__ = Dict{String, Int64}("k1" => 0)
try
foo(1, k1 = 10)
...
function test_kwonly_methods(self::@like(KeywordOnlyArgTestCase))
@test (f(Example(), k1 = 1, k2 = 2) == (1, 2))
@test (f(Example(), k1 = 1, k2 = 2) == (1, 2))
@test_throws
end

function test_issue13343(self::@like(KeywordOnlyArgTestCase))
() -> nothing
end

function test_mangling(self::@like(KeywordOnlyArgTestCase))
@test (f(X()) == 42)
end

function test_default_evaluation_order(self::@like(KeywordOnlyArgTestCase))
a = 42
@test_throws NameError do err 
function f(v::@like(KeywordOnlyArgTestCase) = a, x = b)
#= pass =#
end
