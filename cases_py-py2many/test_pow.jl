...
@oodef mutable struct TestRpow
                    
                    
                    
                end
                function __rpow__(self::@like(TestRpow), other)
return nothing
end


@oodef mutable struct PowTest <: unittest.TestCase
                    
                    
                    
                end
                function powtest(self::@like(PowTest), type_)
if type_ != Float64
for i in -1000:999
...
for othertype in (Int64,)
for i in [collect(-10:-1); collect(1:9)]
ii = type_(i)
for j in 1:10
jj = -othertype(j)
pow(ii, jj)
...
for othertype in (Int64, Float64)
for i in 1:99
zero_ = type_(0)
exp_ = -othertype(i / 10.0)
if exp_ == 0
continue;
end
@test_throws
...
function test_powint(self::@like(PowTest))
powtest(self, Int64)
end

function test_powfloat(self::@like(PowTest))
powtest(self, Float64)
end

function test_other(self::@like(PowTest))
...
if j >= 0&&k != 0
@test (pow(Int(i), j) % k == pow(Int(i), j, k))
end
end
end
end
...
function test_bug643260(self::@like(PowTest))
nothing^TestRpow()
end

function test_bug705231(self::@like(PowTest))
eq = self.assertEqual
a = -1.0
...
function test_negative_exponent(self::@like(PowTest))
for a in -50:49
for m in -50:49
subTest(self, a = a, m = m) do 
if m != 0&&math.gcd(a, m) == 1
inv_ = pow(a, -1, m)
@test (inv_ == inv_ % m)
@test ((inv_*a - 1) % m == 0)
@test (pow(a, -2, m) == pow(inv_, 2, m))
@test (pow(a, -3, m) == pow(inv_, 3, m))
@test (pow(a, -1001, m) == pow(inv_, 1001, m))
else
