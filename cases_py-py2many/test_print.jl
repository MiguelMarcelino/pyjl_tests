...
NotDefined = object()
dispatch = Dict{Tuple{Bool}, Any}((false, false, false) => (args, sep, end_, file) -> println(args...), (false, false, true) => (args, sep, end_, file) -> write(file, "$(args...)"), (false, true, false) => (args, sep, end_, file) -> println("$(args...)"), (false, true, true) => (args, sep, end_, file) -> write(file, "$(args...)"), (true, false, false) => (args, sep, end_, file) -> println("$(args...)"), (true, false, true) => (args, sep, end_, file) -> write(file, "$(args...)"), (true, true, false) => (args, sep, end_, file) -> println("$(args...)"), (true, true, true) => (args, sep, end_, file) -> write(file, "$(args...)"))
@oodef mutable struct ClassWith__str__
...
x = x
                ...
                function __str__(self::@like(ClassWith__str__))
return self.x
end


@oodef mutable struct filelike
                    
                    written::String
flushed::Int64
                    
function new(written::String = "", flushed::Int64 = 0)
@mk begin
written = written
flushed = flushed
end
                ...
                function write(self::@like(filelike), str)
self.written += str
end

function flush(self::@like(filelike))
self.flushed += 1
end


@oodef mutable struct noflush
                    
                    
                    
                end
                function write(self::@like(noflush), str)
#= pass =#
end

function flush(self::@like(noflush))
throw(RuntimeError)
end


@oodef mutable struct TestPrint <: unittest.TestCase
                    #= Test correct operation of the print function. =#

                    
                    
                end
                function check(self::@like(TestPrint), expected, args, sep = NotDefined, end_ = NotDefined, file = NotDefined)
fn = dispatch[(sep !== NotDefined, end_ !== NotDefined, file !== NotDefined)]
support.captured_stdout() do t 
fn(args, sep, end_, file)
end
@test (getvalue(t) == expected)
end

function test_print(self::@like(TestPrint))
function x(expected::@like(TestPrint), args, sep = NotDefined, end_ = NotDefined)
check(self, expected, args, sep = sep, end_ = end_)
o = StringIO()
check(self, "", args, sep = sep, end_ = end_, file = o)
...
x("1 s", (1, "s"), end_ = "")
x("a\nb\n", ("a", "b"), sep = "\n")
x("1.01", (1.0, 1), sep = "", end_ = "")
x("1*a*1.3+", (1, "a", 1.3), sep = "*", end_ = "+")
x("a\n\nb\n", ("a\n", "b"), sep = "\n")
x("\0+ +\0\n", ("\0", " ", "\0"), sep = "+")
x("a\n b\n", ("a\n", "b"))
x("a\n b\n", ("a\n", "b"), sep = nothing)
x("a\n b\n", ("a\n", "b"), end_ = nothing)
x("a\n b\n", ("a\n", "b"), sep = nothing, end_ = nothing)
x("*\n", (ClassWith__str__("*"),))
...
function test_print_flush(self::@like(TestPrint))
f = filelike()
write(f, "1")
flush(f)
write(f, "2")
...
@oodef mutable struct OverrideRRShift
                    
                    
                    
                end
                function __rrshift__(self::@like(OverrideRRShift), lhs)::Int64
return 42
end


@oodef mutable struct TestPy2MigrationHint <: unittest.TestCase
    ...
    if print statement is executed as in Python 2.
     =#
                ...
                function test_normal_string(self::@like(TestPy2MigrationHint))
python2_print_str = "print \"Hello World\""
@test_throws SyntaxError do context 
py"""python2_print_str"""
end
assertIn(self, "Missing parentheses in call to \'print\'. Did you mean print(...)", string(context.exception))
end

function test_string_with_soft_space(self::@like(TestPy2MigrationHint))
python2_print_str = "print \"Hello World\","
@test_throws SyntaxError do context 
py"""python2_print_str"""
end
...
function test_string_with_excessive_whitespace(self::@like(TestPy2MigrationHint))
python2_print_str = "print  \"Hello World\", "
@test_throws SyntaxError do context 
py"""python2_print_str"""
end
...
function test_string_with_leading_whitespace(self::@like(TestPy2MigrationHint))
...
function test_string_with_semicolon(self::@like(TestPy2MigrationHint))
...
function test_string_in_loop_on_same_line(self::@like(TestPy2MigrationHint))
...
function test_stream_redirection_hint_for_py2_migration(self::@like(TestPy2MigrationHint))
@test_throws TypeError do context 
