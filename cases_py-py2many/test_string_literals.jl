...
@oodef mutable struct TestLiterals <: unittest.TestCase
                    
                    save_path
tmpdir
                    
function new(save_path = sys.path[begin:end], tmpdir = tempfile.mkdtemp())
save_path = save_path
tmpdir = tmpdir
new(save_path, tmpdir)
end

                end
                function setUp(self::@like(TestLiterals))
self.save_path = sys.path[begin:end]
self.tmpdir = tempfile.mkdtemp()
sys.insert(0, self.tmpdir)
end

function tearDown(self::@like(TestLiterals))
sys.path[begin:end] = self.save_path
Base.Filesystem.rm(self.tmpdir, recursive=True)
end

function test_template(self::@like(TestLiterals))
for c in TEMPLATE
@assert(c == "\n"||" " <= c <= "~")
end
end

function test_eval_str_normal(self::@like(TestLiterals))
@test (py"" \'x\' "" == "x")
@test (py"" \'\\x01\' "" == Char(1))
@test (py"" \'\' "" == Char(1))
@test (py"" \'\\x81\' "" == Char(129))
@test (py"" \'\' "" == Char(129))
@test (py"" \'\\u1881\' "" == Char(6273))
@test (py"" \'ᢁ\' "" == Char(6273))
@test (py"" \'\\U0001d120\' "" == Char(119072))
@test (py"" \'𝄠\' "" == Char(119072))
end

function test_eval_str_incomplete(self::@like(TestLiterals))
@test_throws
@test_throws
@test_throws
@test_throws
...
function test_eval_str_invalid_escape(self::@like(TestLiterals))
for b in 1:127
...
warnings.catch_warnings(record = true) do w 
...
function test_eval_str_raw(self::@like(TestLiterals))
@test (py"" r\'x\' "" == "x")
@test (py"" r\'\\x01\' "" == "\\" * "x01")
@test (py"" r\'\' "" == Char(1))
@test (py"" r\'\\x81\' "" == "\\" * "x81")
@test (py"" r\'\' "" == Char(129))
@test (py"" r\'\\u1881\' "" == "\\" * "u1881")
@test (py"" r\'ᢁ\' "" == Char(6273))
@test (py"" r\'\\U0001d120\' "" == "\\" * "U0001d120")
@test (py"" r\'𝄠\' "" == Char(119072))
end

function test_eval_bytes_normal(self::@like(TestLiterals))
@test (py"" b\'x\' "" == b"x")
@test (py"" b\'\\x01\' "" == byte(1))
@test (py"" b\'\' "" == byte(1))
@test (py"" b\'\\x81\' "" == byte(129))
@test_throws
@test (py"" br\'\\u1881\' "" == [b"\\"; b"u1881"])
@test_throws
@test (py"" br\'\\U0001d120\' "" == [b"\\"; b"U0001d120"])
@test_throws
end

function test_eval_bytes_incomplete(self::@like(TestLiterals))
@test_throws
@test_throws
end

function test_eval_bytes_invalid_escape(self::@like(TestLiterals))
for b in 1:127
if b ∈ b"\n\r\"\'01234567\\abfnrtvx"
continue;
end
assertWarns(self, DeprecationWarning) do 
@test (py""b\'\\$(b)\'"" == [b"\\"; bytes([b])])
end
end
warnings.catch_warnings(record = true) do w 
warnings.simplefilter("always", category = DeprecationWarning)
...
function test_eval_bytes_raw(self::@like(TestLiterals))
@test (py"" br\'x\' "" == b"x")
@test (py"" rb\'x\' "" == b"x")
@test (py"" br\'\\x01\' "" == [b"\\"; b"x01"])
@test (py"" rb\'\\x01\' "" == [b"\\"; b"x01"])
@test (py"" br\'\' "" == byte(1))
@test (py"" rb\'\' "" == byte(1))
@test (py"" br\'\\x81\' "" == [b"\\"; b"x81"])
@test (py"" rb\'\\x81\' "" == [b"\\"; b"x81"])
@test_throws
@test_throws
@test (py"" br\'\\u1881\' "" == [b"\\"; b"u1881"])
@test (py"" rb\'\\u1881\' "" == [b"\\"; b"u1881"])
@test_throws
@test_throws
@test (py"" br\'\\U0001d120\' "" == [b"\\"; b"U0001d120"])
@test (py"" rb\'\\U0001d120\' "" == [b"\\"; b"U0001d120"])
@test_throws
@test_throws
@test_throws
@test_throws
...
function test_eval_str_u(self::@like(TestLiterals))
@test (py"" u\'x\' "" == "x")
@test (py"" U\'ä\' "" == "ä")
@test (py"" u\'ä\' "" == "ä")
@test_throws
...
function check_encoding(self::@like(TestLiterals), encoding, extra = "")
modname = "xx_" + replace(encoding, "-", "_")
fn = os.join(self.tmpdir, modname + ".py")
f = readline(fn)
try
...
function test_file_utf_8(self::@like(TestLiterals))
extra = "z = \'ሴ\'; assert ord(z) == 0x1234\n"
check_encoding(self, "utf-8", extra)
end

function test_file_utf_8_error(self::@like(TestLiterals))
extra = "b\'\x80\'\n"
@test_throws
end

function test_file_utf8(self::@like(TestLiterals))
check_encoding(self, "utf-8")
end

function test_file_iso_8859_1(self::@like(TestLiterals))
check_encoding(self, "iso-8859-1")
end

function test_file_latin_1(self::@like(TestLiterals))
check_encoding(self, "latin-1")
end

function test_file_latin9(self::@like(TestLiterals))
