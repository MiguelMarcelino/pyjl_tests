...
@oodef mutable struct Y
                ...
                function __eq__(self::@like(Y), other)
clear(list1)
return NotImplemented
end


@oodef mutable struct Z
                    
                    
                    
                end
                function __eq__(self::@like(Z), other)
clear(list3)
return NotImplemented
end
...
@oodef mutable struct L <: Vector
                ...
                function __eq__(self::@like(L), other)
string(other)
return NotImplemented
end


@oodef mutable struct ListTest <: list_tests.CommonTest
                    
                    type2test
                    
function new(type2test = Vector)
type2test = type2test
new(type2test)
end

                end
                function test_basic(self::@like(ListTest))
@test (collect([]) == [])
l0_3 = [0, 1, 2, 3]
...
function test_keyword_args(self::@like(ListTest))
assertRaisesRegex(self, TypeError, "keyword argument") do 
Vector()
end
end

function test_truth(self::@like(ListTest))
list_tests.CommonTest()
@test !([])
@test [42]
end

function test_identity(self::@like(ListTest))
@test [] !== []
end

function test_len(self::@like(ListTest))
list_tests.CommonTest()
@test (length([]) == 0)
@test (length([0]) == 1)
@test (length([0, 1, 2]) == 3)
...
function test_overflow(self::@like(ListTest))
lst = [4, 5, 6, 7]
n = parse(Int, (typemax(Int)*2 + 2) ÷ length(lst))
function mul(a::@like(ListTest), b)
return a*b
end

function imul(a::@like(ListTest), b)
a *= b
end
...
function test_repr_large(self::@like(ListTest))
function check(n::@like(ListTest))
l = [0]*n
s = repr(l)
@test (s == ("[" + join(["0"]*n, ", ")) + "]")
end
...
function test_iterator_pickle(self::@like(ListTest))
...
function test_reversed_pickle(self::@like(ListTest))
orig = type2test(self, [4, 5, 6, 7])
data = [10, 11, 12, 13, 14, 15]
for proto in 0:pickle.HIGHEST_PROTOCOL
...
function test_step_overflow(self::@like(ListTest))
a = [0, 1, 2, 3, 4]
a[2:typemax(Int):end] = [0]
@test (a[4:typemax(Int):end] == [3])
end

function test_no_comdat_folding(self::@like(ListTest))
@test_throws TypeError do 
(3,) + L([1, 2])
end
end

function test_equal_operator_modifying_operand(self::@like(ListTest))
list1 = [X()]
list2 = [Y()]
@test list1 == list2
list3 = [Z()]
...
function test_preallocation(self::@like(ListTest))
iterable = repeat([0],10)
iter_size = sys.getsizeof(iterable)
@test (iter_size == sys.getsizeof(collect(repeat([0],10))))
@test (iter_size == sys.getsizeof(collect(0:9)))
end

function test_count_index_remove_crashes(self::@like(ListTest))
lst = [X()]
