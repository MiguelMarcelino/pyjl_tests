...
@oodef mutable struct HalibutProxy
                    
                    
                    
                end
                function __hash__(self::@like(HalibutProxy))
return hash("halibut")
end

function __eq__(self::@like(HalibutProxy), other)::Bool
return other == "halibut"
end


@oodef mutable struct HashTest <: unittest.TestCase
                    
                    
                    
                end
                function check_equal_hash(self::@like(HashTest), x, y)
@test (hash(x) == hash(y))
@test (x == y)
end

function test_bools(self::@like(HashTest))
check_equal_hash(self, false, 0)
check_equal_hash(self, true, 1)
end

function test_integers(self::@like(HashTest))
for i in -1000:999
...
function test_binary_floats(self::@like(HashTest))
check_equal_hash(self, 0.0, -0.0)
check_equal_hash(self, 0.0, D(0))
check_equal_hash(self, -0.0, D(0))
check_equal_hash(self, -0.0, D("-0.0"))
check_equal_hash(self, 0.0, F(0))
check_equal_hash(self, parse(Float64, "inf"), D("inf"))
check_equal_hash(self, parse(Float64, "-inf"), D("-inf"))
for _ in 0:999
x = rand()*math.exp(rand()*200.0 - 100.0)
check_equal_hash(self, x, from_float(D, x))
check_equal_hash(self, x, from_float(F, x))
end
end

function test_complex(self::@like(HashTest))
test_values = [0.0, -0.0, 1.0, -1.0, 0.40625, -5136.5, parse(Float64, "inf"), parse(Float64, "-inf")]
for zero_ in (-0.0, 0.0)
for value in test_values
check_equal_hash(self, value, complex(value, zero_))
end
end
end

function test_decimals(self::@like(HashTest))
zeros_ = ["0", "-0", "0.0", "-0.0e10", "000e-10"]
for zero_ in zeros_
check_equal_hash(self, D(zero_), D(0))
end
...
function test_fractions(self::@like(HashTest))
...
function test_hash_normalization(self::@like(HashTest))
...
@oodef mutable struct ComparisonTest <: unittest.TestCase
                ...
                function test_mixed_comparisons(self::@like(ComparisonTest))
test_values = [parse(Float64, "-inf"), D("-1e425000000"), -1e+308, F(-22, 7), -3.14, -2, 0.0, 1e-320, true, F("1.2"), D("1.3"), parse(Float64, "1.4"), F(275807, 195025), D("1.414213562373095048801688724"), F(114243, 80782), F(473596569, 84615), 7e+200, D("infinity")]
for (i, first_) in enumerate(test_values)
for second in test_values[i + 2:end]
assertLess(self, first_, second)
assertLessEqual(self, first_, second)
assertGreater(self, second, first_)
assertGreaterEqual(self, second, first_)
end
end
end

function test_complex(self::@like(ComparisonTest))
z = 1.0 + 0im
w = -3.14 + 2.7im
