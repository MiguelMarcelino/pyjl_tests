...
@resumable function pyrange(start, stop, step)
...
@oodef mutable struct BadExc <: Exception
...
@oodef mutable struct BadCmp
                ...
                function __eq__(self::@like(BadCmp), other)::Bool
if other == 2
throw(BadExc())
end
return false
...
n = n
                ...
                function __index__(self::@like(I))::Int64
return self.n
end


@oodef mutable struct IX
                    
                    
                    
                end
                function __index__(self::@like(IX))
throw(RuntimeError)
end


@oodef mutable struct IN
                    
                    
                    
                end
                function __index__(self::@like(IN))::String
return "not a number"
end


@oodef mutable struct C2
                    
                    
                    
                end
                function __int__(self::@like(C2))::Int64
return 1
end

function __index__(self::@like(C2))::Int64
return 1
end


@oodef mutable struct C3 <: Int64
                    
                    
                    
                end
                function __eq__(self::@like(C3), other)::Bool
return true
end


@oodef mutable struct RangeTest <: unittest.TestCase
                    
                    
                    
                end
                function assert_iterators_equal(self::@like(RangeTest), xs, ys, test_id, limit = nothing)
if limit !== nothing
xs = (xs for _ in (1:limit))
ys = (ys for _ in (1:limit))
end
sentinel = object()
pairs_ = itertools.zip_longest(xs, ys, fillvalue = sentinel)
for (i, (x, y)) in enumerate(pairs_)
if x == y
continue;
elseif x == sentinel
fail(self, "$(test_id): iterator ended unexpectedly at position $(i); expected $(y)")
elseif y == sentinel
fail(self, "$(test_id): unexpected excess element $(x) at position $(i)")
else
fail(self, "$(test_id): wrong element at position $(i); expected $(y), got $(x)")
end
end
end

function test_range(self::@like(RangeTest))
...
@test_throws
@test_throws
@test (length(0:typemax(Int) - 1:typemax(Int) - 1) == 2)
r = -(typemax(Int)):2:typemax(Int) - 1
@test (length(r) == typemax(Int))
end

function test_range_constructor_error_messages(self::@like(RangeTest))
...
function test_large_operands(self::@like(RangeTest))
x = 10^20:3:10^20 + 9
@test (length(x) == 4)
...
a = parse(Int, 10*typemax(Int))
b = parse(Int, 100*typemax(Int))
c = parse(Int, 50*typemax(Int))
@test (collect(a:a + 1) == [a, a + 1])
@test (collect(a - 1:-1:a + 2) == [a + 2, a + 1])
@test (collect(a - 1:-2:a + 4) == [a + 4, a + 2])
seq = collect(a:c:b - 1)
...
function test_large_range(self::@like(RangeTest))
function _range_len(x::@like(RangeTest))::Int64
try
length_ = length(x)
catch exn
if exn isa OverflowError
step_ = x[2] - x[1]
length_ = 1 + ((x[end] - x[1]) ÷ step_)
end
end
return length_
end
...
function test_invalid_invocation(self::@like(RangeTest))
@test_throws
@test_throws
@test_throws
a = parse(Int, 10*typemax(Int))
...
function test_index(self::@like(RangeTest))
u = 0:1
@test (index(u, 0) == 0)
@test (index(u, 1) == 1)
@test_throws
...
a = -2:2
@test (index(a, 0) == 2)
@test (index(1:3:9, 4) == 1)
@test (index(-9:-3:1, -5) == 2)
@test (index(0:10^20, 1) == 1)
@test (index(0:10^20, 10^20 - 1) == 10^20 - 1)
@test_throws
@test (index(1:2:2^100, 2^87 + 1) == 2^86)
@test (index(0:9, ALWAYS_EQ) == 0)
end

function test_user_index_method(self::@like(RangeTest))
bignum = 2*typemax(Int)
smallnum = 42
@test (collect(I(bignum):I(bignum + 1) - 1) == [bignum])
@test (collect(I(smallnum):I(smallnum + 1) - 1) == [smallnum])
@test_throws
@test_throws
@test (0:9[begin:I(5)] == 0:4)
...
function test_count(self::@like(RangeTest))
@test (count(0:2, -1) == 0)
@test (count(0:2, 0) == 1)
@test (count(0:2, 1) == 1)
@test (count(0:2, 2) == 1)
@test (count(0:2, 3) == 0)
@test self === type_(count(0:2, -1))
@test self === type_(count(0:2, 1))
...
@test (length(typemax(Int):typemax(Int) + 9) == 10)
end

function test_repr(self::@like(RangeTest))
@test (repr(0:0) == "range(0, 1)")
@test (repr(1:1) == "range(1, 2)")
@test (repr(1:3:1) == "range(1, 2, 3)")
end

function test_pickling(self::@like(RangeTest))
testcases = [(13,), (0, 11), (-22, 10), (20, 3, -1), (13, 21, 3), (-2, 2, 2), (2^65, 2^65 + 2)]
for proto in 0:pickle.HIGHEST_PROTOCOL
for t in testcases
subTest(self, proto = proto, test = t) do 
r = 0:t... - 1
@test (collect(pickle.loads(pickle.dumps(r, proto))) == collect(r))
end
end
end
end

function test_iterator_pickling(self::@like(RangeTest))
testcases = [(13,), (0, 11), (-22, 10), (20, 3, -1), (13, 21, 3), (-2, 2, 2)]
for M in (2^31, 2^63)
append!(testcases, [(M - 3, M - 1), (4*M, 4*M + 2), (M - 2, M - 1, 2), (-M + 1, -M, -2), (1, 2, M - 1), (-1, -2, -M), (1, M - 1, M - 1), (-1, -M, -M)])
end
for proto in 0:pickle.HIGHEST_PROTOCOL
for t in testcases
subTest(self, proto = proto, t = t) do 
it=itorg = (x for x in 0:t... - 1)
data = collect(0:t... - 1)
...
function test_iterator_pickling_overflowing_index(self::@like(RangeTest))
for proto in 0:pickle.HIGHEST_PROTOCOL
subTest(self, proto = proto) do 
it = (x for x in 0:2^32 + 1)
...
function test_exhausted_iterator_pickling(self::@like(RangeTest))
for proto in 0:pickle.HIGHEST_PROTOCOL
r = 2^65:2^65 + 1
i = (x for x in r)
while true
...
function test_large_exhausted_iterator_pickling(self::@like(RangeTest))
for proto in 0:pickle.HIGHEST_PROTOCOL
r = 0:19
i = (x for x in r)
while true
...
function test_odd_bug(self::@like(RangeTest))
@test_throws TypeError do 
0:-1:[]
end
end

function test_types(self::@like(RangeTest))
...
function test_strided_limits(self::@like(RangeTest))
r = 0:2:100
assertIn(self, 0, r)
...
function test_empty(self::@like(RangeTest))
r = 0:-1
...
function test_range_iterators(self::@like(RangeTest))
...
for (start, end_, step_) in test_ranges
iter1 = start:step_:end_ - 1
iter2 = pyrange(start, end_, step_)
test_id = "range($(start), $(end_), $(step_))"
assert_iterators_equal(self, iter1, iter2, test_id, limit = 100)
iter1 = reversed(start:step_:end_ - 1)
iter2 = pyrange_reversed(start, end_, step_)
test_id = "reversed(range($(start), $(end_), $(step_)))"
assert_iterators_equal(self, iter1, iter2, test_id, limit = 100)
end
end

function test_range_iterators_invocation(self::@like(RangeTest))
...
function test_slice(self::@like(RangeTest))
function check(start::@like(RangeTest), stop, step = nothing)
i = (start:stop)
@test (collect(r[i + 1]) == collect(r)[i + 1])
@test (length(r[i + 1]) == length(collect(r)[i + 1]))
end

for r in [0:9, 0:-1, 1:3:8, -1:-3:8, typemax(Int) + 1:typemax(Int) + 9]
check(0, 2)
check(0, 20)
check(1, 2)
check(20, 30)
...
function test_contains(self::@like(RangeTest))
r = 0:9
assertIn(self, 0, r)
assertIn(self, 1, r)
assertIn(self, 5.0, r)
...
function test_reverse_iteration(self::@like(RangeTest))
for r in [0:9, 0:-1, 1:3:8, -1:-3:8, typemax(Int) + 1:typemax(Int) + 9]
@test (collect(reversed(r)) == collect(r)[end:-1:begin])
end
end

function test_issue11845(self::@like(RangeTest))
r = 0:indices((1:18), 20)... - 1
values_ = Set([nothing, 0, 1, -1, 2, -2, 5, -5, 19, -19, 20, -20, 21, -21, 30, -30, 99, -99])
for i in values_
for j in values_
for k in values_ - Set([0])
r[i + 1:k:j]
end
end
end
...
function test_comparison(self::@like(RangeTest))
...
@test self === () == 0:-1
@test self === 0:1 == [0, 1]
@test (0:2:2^100 - 2 == 0:2:2^100)
@test (hash(0:2:2^100 - 2) == hash(0:2:2^100))
...
@test (2^200:2^100:2^201 != 2^200:2^100:2^201)
@test_throws TypeError do 
0:-1 < 0:-1
end
@test_throws TypeError do 
...
function test_attributes(self::@like(RangeTest))
...
function assert_attrs(self::@like(RangeTest), rangeobj, start, stop, step)
@test (rangeobj.start == start)
@test (rangeobj.stop == stop)
@test (rangeobj.step_ == step)
@test self === type_(rangeobj.start)
@test self === type_(rangeobj.stop)
@test self === type_(rangeobj.step_)
