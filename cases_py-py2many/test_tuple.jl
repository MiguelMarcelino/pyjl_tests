...
@oodef mutable struct MyTuple <: Tuple
...
@oodef mutable struct TupleTest <: seq_tests.CommonTest
                    
                    type2test
                    
function new(type2test = Tuple)
type2test = type2test
new(type2test)
end

                end
                function test_getitem_error(self::@like(TupleTest))
t = ()
...
function test_constructors(self::@like(TupleTest))
...
@test t0_3 === t0_3_bis
@test (tuple([]) == ())
@test (tuple([0, 1, 2, 3]) == (0, 1, 2, 3))
@test (tuple("") == ())
@test (tuple("spam") == ("s", "p", "a", "m"))
@test (tuple((x for x in 0:9 if x % 2 )...) == (1, 3, 5, 7, 9))
end

function test_keyword_args(self::@like(TupleTest))
assertRaisesRegex(self, TypeError, "keyword argument") do 
tuple(sequence = ())
end
end

function test_truth(self::@like(TupleTest))
seq_tests.CommonTest()
@test !(())
@test (42,)
end

function test_len(self::@like(TupleTest))
seq_tests.CommonTest()
@test (length(()) == 0)
@test (length((0,)) == 1)
@test (length((0, 1, 2)) == 3)
...
function test_iadd(self::@like(TupleTest))
seq_tests.CommonTest()
u = (0, 1)
u2 = u
u += (2, 3)
...
function test_imul(self::@like(TupleTest))
seq_tests.CommonTest()
u = (0, 1)
u2 = u
u *= 3
...
@resumable function test_tupleresizebug(self::@like(TupleTest))
@test (collect(tuple(f())) == collect(0:999))
end

function test_hash_exact(self::@like(TupleTest))
function check_one_exact(t::@like(TupleTest), e32, e64)
got = hash(t)
...
function test_hash_optional(self::@like(TupleTest))
if !RUN_ALL_HASH_TESTS
return
end
function tryone_inner(tag::@like(TupleTest), nbins, hashes, expected = nothing, zlimit = nothing)
...
function tryone(tag::@like(TupleTest), xs, native32 = nothing, native64 = nothing, hi32 = nothing, lo32 = nothing, zlimit = nothing)
...
cands = [collect(-10:0); collect(0:8)]
...
tryone("4-char tuples", collect(product("abcdefghijklmnopqrstuvwxyz", repeat = 4)), zlimit = 4.0)
N = 50
base = collect(0:N - 1)
xp = collect(product(base, repeat = 2))
inps = [[[[base; collect(product(base, xp))]; collect(product(xp, base))]; xp]; collect(zip(base))]
...
L3 = [L2; collect(product(A, repeat = 3))]
L4 = [L3; collect(product(A, repeat = 4))]
T = A
T += [(a,) for a in B + L4]
T += product(L3, B)
...
function test_repr(self::@like(TupleTest))
l0 = tuple()
l2 = (0, 1, 2)
a0 = type2test(self, l0)
a2 = type2test(self, l2)
@test (string(a0) == repr(l0))
@test (string(a2) == repr(l2))
@test (repr(a0) == "()")
@test (repr(a2) == "(0, 1, 2)")
end

function _not_tracked(self::@like(TupleTest), t)
gc.collect()
gc.collect()
@test !(gc.is_tracked(t))
end

function _tracked(self::@like(TupleTest), t)
@test gc.is_tracked(t)
gc.collect()
gc.collect()
@test gc.is_tracked(t)
...
function test_track_literals(self::@like(TupleTest))
(x, y, z) = (1.5, "a", [])
_not_tracked(self, ())
_not_tracked(self, (1,))
_not_tracked(self, (1, 2))
_not_tracked(self, (1, 2, "a"))
_not_tracked(self, (1, 2, (nothing, true, false, ()), Int64))
...
function check_track_dynamic(self::@like(TupleTest), tp, always_track)
(x, y, z) = (1.5, "a", [])
...
function test_track_dynamic(self::@like(TupleTest))
check_track_dynamic(self, Tuple, false)
end

function test_track_subtypes(self::@like(TupleTest))
check_track_dynamic(self, MyTuple, true)
end

function test_bug7466(self::@like(TupleTest))
_not_tracked(self, tuple((gc.collect() for i in 0:100)...))
end

function test_repr_large(self::@like(TupleTest))
function check(n::@like(TupleTest))
l = (0,)*n
s = repr(l)
@test (s == ("(" + join(["0"]*n, ", ")) + ")")
end
...
function test_iterator_pickle(self::@like(TupleTest))
...
@test (type2test(self, it) == type2test(self, data))
it = pickle.loads(d)
next(it)
d = pickle.dumps(it, proto)
@test (type2test(self, it) == type2test(self, data)[2:end])
end
end

function test_reversed_pickle(self::@like(TupleTest))
data = type2test(self, [4, 5, 6, 7])
for proto in 0:pickle.HIGHEST_PROTOCOL
itorg = reversed(data)
d = pickle.dumps(itorg, proto)
it = pickle.loads(d)
@test (type_(itorg) == type_(it))
@test (type2test(self, it) == type2test(self, reversed(data)))
it = pickle.loads(d)
next(it)
d = pickle.dumps(it, proto)
@test (type2test(self, it) == type2test(self, reversed(data))[2:end])
end
end

function test_no_comdat_folding(self::@like(TupleTest))
@test_throws TypeError do 
[3] + T((1, 2))
end
end

function test_lexicographic_ordering(self::@like(TupleTest))
