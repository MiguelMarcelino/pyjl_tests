...
abstract type AbstractB <: bytes end
abstract type AbstractBytesTest <: AbstractBaseBytesTest end
abstract type AbstractIterationBlocked <: list end
abstract type AbstractIntBlocked <: int end
abstract type AbstractBytesSubclassBlocked <: bytes end
abstract type AbstractBufferBlocked <: bytearray end
abstract type AbstractSubBytes <: bytes end
...
abstract type AbstractB1 <: self.basetype end
abstract type AbstractB2 <: self.basetype end
abstract type AbstractByteArraySubclass <: bytearray end
abstract type AbstractBytesSubclass <: bytes end
abstract type AbstractOtherBytesSubclass <: bytes end
abstract type AbstractByteArraySubclassTest <: AbstractSubclassTest end
abstract type Abstractsubclass <: bytearray end
...
@test (PyBytes_FromFormat(b"c=%c", c_int(Int(codepoint('@')))) == b"c=@")
...
ptr = 11259375
@test (PyBytes_FromFormat(b"ptr=%p", c_char_p(ptr)) == encode("ptr=" + ptr_formatter(ptr), "ascii"))
...
(@test (PyBytes_FromFormat(formatstr, ctypes_type(value)) == encode(py_formatter(value), "ascii")),)
end
@test (PyBytes_FromFormat(b"%5s", b"a") == b"a")
@test (PyBytes_FromFormat(b"%.3s", b"abcdef") == b"abc")
@test (PyBytes_FromFormat(b"%%") == b"%")
@test (PyBytes_FromFormat(b"[%%]") == b"[%]")
@test (PyBytes_FromFormat(b"%%%c", c_int(Int(codepoint('_')))) == b"%_")
...
remove(b, Int(codepoint('l')))
@test (b == b"heo")
@test_throws ValueError () -> remove(b, Int(codepoint('l')))()
...
@test (b == string(b))
@test (b == string(b))
assertRaisesRegex(self, TypeError, "\\b%s\\b" % escape(type_(b).__name__)) do 
b
end
end
end
...
for c in 128:255
@test_throws SyntaxError eval("b\"%s\"" % Char(c))
end
end
...
assertRaises(self, ValueError, self.type2test.fromhex, "rt")
assertRaises(self, ValueError, self.type2test.fromhex, "1a b cd")
assertRaises(self, ValueError, self.type2test.fromhex, "\000")
assertRaises(self, ValueError, self.type2test.fromhex, "12   \000   34")
...
assertEqual(self, hex(three_bytes, ":", 0), "b901ef")
assertEqual(self, hex(three_bytes, b"\x00"), "b9\00001\000ef")
assertEqual(self, hex(three_bytes, "\000"), "b9\00001\000ef")
assertEqual(self, hex(three_bytes, b"\x7f"), "b901ef")
assertEqual(self, hex(three_bytes, ""), "b901ef")
assertEqual(self, hex(three_bytes, ":", 3), "b901ef")
assertEqual(self, hex(three_bytes, ":", 4), "b901ef")
...
function test_buffer_is_readonly(self::BytesTest)
fd = readline(os)(__file__, os.O_RDONLY)
readline(fd) do f 
@test_throws TypeError f.readinto(b"")
end
end
...
#Delete Unsupported
del(b)
assertLessEqual(self, sys.getsizeof(b), size)
end
...
@test (b == string(b))
@test (b == string(b))
assertRaisesRegex(self, TypeError, "\\b%s\\b" % re.escape(type_(b).__name__)) do 
b
end
end
end
...
function test_literal(self::AssortedBytesTest)
tests = [(b"Wonderful spam", "Wonderful spam"), (b"Wonderful spam too", "Wonderful spam too"), (b"\xaa\x00\x00\x80", "ª\000\000"), (b"\\xaa\\x00\\000\\200", "\\xaa\\x00\\000\\200")]
for (b, s) in tests
@test (b == Vector{UInt8}(s))
end
for c in 128:255
