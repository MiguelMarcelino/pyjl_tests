...
abstract type AbstractMyContextVar <: contextvars.ContextVar end
abstract type AbstractMyContext <: contextvars.Context end
abstract type AbstractMyToken <: contextvars.Token end
abstract type AbstractKeyStr <: str end
abstract type AbstractHashingError <: Exception end
abstract type AbstractEqError <: Exception end
...
assertIn(self, "a", repr(c))
c = contextvars.ContextVar("a", 123)
assertIn(self, "123", repr(c))
lst = []
c = contextvars.ContextVar("a", lst)
push!(lst, c)
assertIn(self, "...", repr(c))
assertIn(self, "...", repr(lst))
t = set(c, 1)
...
assertRaisesRegex(self, TypeError, "any arguments") do 
contextvars.Context(1, 1)
end
assertRaisesRegex(self, TypeError, "any arguments") do 
contextvars.Context(1)
end
contextvars.Context(Dict())
end
...
run(contextvars.copy_context(), fun)
@test (get(c) == [])
end
...
tp = concurrent.futures.ThreadPoolExecutor(10)
try
results = collect(map(tp, sub, 0:9))
finally
shutdown(tp)
