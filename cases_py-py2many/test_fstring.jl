...
using test.support.os_helper: temp_cwd
using test.support.script_helper: assert_python_failure
a_global = "global variable"
@resumable function fn(y::TestCase)
"y:$(@yield y*2)"
"$(@yield)"
end

@resumable function fn(x::TestCase)
@yield "x:$(@yield (i) -> x*i)"
end
...
@oodef mutable struct X
                ...
                function __format__(self::@like(X), spec)::String
return "class"
end
...
function new(called::Bool = false)
@mk begin
called = called
end
end
                ...
                function __call__(self::@like(X))::Int64
self.called = true
return 4
end
...
@oodef mutable struct O
                    
                    
                    
                end
                function __format__(self::@like(O), spec)
if !spec
return "*"
end
return spec
...
@oodef mutable struct C
                ...
                function __format__(self::@like(C), s)
return "FORMAT-$(s)"
end
...
@oodef mutable struct TestCase <: unittest.TestCase
                ...
                function assertAllRaise(self::@like(TestCase), exception_type, regex, error_strings)
for str in error_strings
subTest(self, str = str) do 
assertRaisesRegex(self, exception_type, regex) do 
py"str"
end
end
end
...
function test__format__lookup(self::@like(TestCase))
x = X()
y = X()
y.__format__ = types.MethodType((self, spec) -> "instance", y)
@test ("$(y)" == y)
@test ("$(y)" == "class")
@test (x == y)
@test (__format__(x, "") == "class")
@test (__format__(y, "") == "instance")
@test (__format__(type_(x), x, "") == "class")
@test (__format__(type_(y), y, "") == "class")
end

function test_ast(self::@like(TestCase))
x = X()
expr = "\na = 10\nf\'{a * x()}\'"
t = ast.parse(expr)
c = compile(t, "", "exec")
...
function test_ast_line_numbers(self::@like(TestCase))
expr = "\na = 10\nf\'{a * x()}\'"
t = ast.parse(expr)
@test (type_(t) == ast.Module)
@test (length(t.body) == 2)
@test (type_(t.body[1]) == ast.Assign)
@test (t.body[1].lineno == 2)
@test (type_(t.body[2]) == ast.Expr)
@test (type_(t.body[2].value) == ast.JoinedStr)
@test (length(t.body[2].value.values) == 1)
@test (type_(t.body[2].value.values[1]) == ast.FormattedValue)
@test (t.body[2].lineno == 3)
@test (t.body[2].value.lineno == 3)
@test (t.body[2].value.values[1].lineno == 3)
binop = t.body[2].value.values[1].value
@test (type_(binop) == ast.BinOp)
@test (type_(binop.left) == ast.Name)
@test (type_(binop.op) == ast.Mult)
@test (type_(binop.right) == ast.Call)
@test (binop.lineno == 3)
@test (binop.left.lineno == 3)
@test (binop.right.lineno == 3)
@test (binop.col_offset == 3)
@test (binop.left.col_offset == 3)
@test (binop.right.col_offset == 7)
end

function test_ast_line_numbers_multiple_formattedvalues(self::@like(TestCase))
expr = "\nf\'no formatted values\'\nf\'eggs {a * x()} spam {b + y()}\'"
t = ast.parse(expr)
@test (type_(t) == ast.Module)
@test (length(t.body) == 2)
@test (type_(t.body[1]) == ast.Expr)
@test (type_(t.body[1].value) == ast.JoinedStr)
@test (t.body[1].lineno == 2)
@test (type_(t.body[2]) == ast.Expr)
@test (type_(t.body[2].value) == ast.JoinedStr)
@test (length(t.body[2].value.values) == 4)
@test (type_(t.body[2].value.values[1]) == ast.Constant)
@test (type_(t.body[2].value.values[1].value) == String)
@test (type_(t.body[2].value.values[2]) == ast.FormattedValue)
@test (type_(t.body[2].value.values[3]) == ast.Constant)
@test (type_(t.body[2].value.values[3].value) == String)
@test (type_(t.body[2].value.values[4]) == ast.FormattedValue)
@test (t.body[2].lineno == 3)
@test (t.body[2].value.lineno == 3)
@test (t.body[2].value.values[1].lineno == 3)
@test (t.body[2].value.values[2].lineno == 3)
@test (t.body[2].value.values[3].lineno == 3)
@test (t.body[2].value.values[4].lineno == 3)
binop1 = t.body[2].value.values[2].value
@test (type_(binop1) == ast.BinOp)
@test (type_(binop1.left) == ast.Name)
@test (type_(binop1.op) == ast.Mult)
@test (type_(binop1.right) == ast.Call)
@test (binop1.lineno == 3)
@test (binop1.left.lineno == 3)
@test (binop1.right.lineno == 3)
@test (binop1.col_offset == 8)
@test (binop1.left.col_offset == 8)
@test (binop1.right.col_offset == 12)
binop2 = t.body[2].value.values[4].value
@test (type_(binop2) == ast.BinOp)
@test (type_(binop2.left) == ast.Name)
@test (type_(binop2.op) == ast.Add)
@test (type_(binop2.right) == ast.Call)
@test (binop2.lineno == 3)
@test (binop2.left.lineno == 3)
@test (binop2.right.lineno == 3)
@test (binop2.col_offset == 23)
@test (binop2.left.col_offset == 23)
@test (binop2.right.col_offset == 27)
end

function test_ast_line_numbers_nested(self::@like(TestCase))
expr = "\na = 10\nf\'{a * f\"-{x()}-\"}\'"
t = ast.parse(expr)
@test (type_(t) == ast.Module)
@test (length(t.body) == 2)
...
@test (type_(binop.left) == ast.Name)
@test (type_(binop.op) == ast.Mult)
@test (type_(binop.right) == ast.JoinedStr)
@test (binop.lineno == 3)
@test (binop.left.lineno == 3)
@test (binop.right.lineno == 3)
@test (binop.col_offset == 3)
@test (binop.left.col_offset == 3)
@test (binop.right.col_offset == 7)
@test (length(binop.right.values) == 3)
@test (type_(binop.right.values[1]) == ast.Constant)
@test (type_(binop.right.values[1].value) == String)
@test (type_(binop.right.values[2]) == ast.FormattedValue)
@test (type_(binop.right.values[3]) == ast.Constant)
@test (type_(binop.right.values[3].value) == String)
@test (binop.right.values[1].lineno == 3)
@test (binop.right.values[2].lineno == 3)
@test (binop.right.values[3].lineno == 3)
call = binop.right.values[2].value
@test (type_(call) == ast.Call)
@test (call.lineno == 3)
@test (call.col_offset == 11)
end

function test_ast_line_numbers_duplicate_expression(self::@like(TestCase))
expr = "\na = 10\nf\'{a * x()} {a * x()} {a * x()}\'\n"
t = ast.parse(expr)
@test (type_(t) == ast.Module)
@test (length(t.body) == 2)
...
@test (type_(t.body[2]) == ast.Expr)
@test (type_(t.body[2].value) == ast.JoinedStr)
@test (length(t.body[2].value.values) == 5)
@test (type_(t.body[2].value.values[1]) == ast.FormattedValue)
@test (type_(t.body[2].value.values[2]) == ast.Constant)
@test (type_(t.body[2].value.values[2].value) == String)
@test (type_(t.body[2].value.values[3]) == ast.FormattedValue)
@test (type_(t.body[2].value.values[4]) == ast.Constant)
@test (type_(t.body[2].value.values[4].value) == String)
@test (type_(t.body[2].value.values[5]) == ast.FormattedValue)
@test (t.body[2].lineno == 3)
@test (t.body[2].value.lineno == 3)
@test (t.body[2].value.values[1].lineno == 3)
@test (t.body[2].value.values[2].lineno == 3)
@test (t.body[2].value.values[3].lineno == 3)
@test (t.body[2].value.values[4].lineno == 3)
@test (t.body[2].value.values[5].lineno == 3)
binop = t.body[2].value.values[1].value
@test (type_(binop) == ast.BinOp)
@test (type_(binop.left) == ast.Name)
@test (type_(binop.op) == ast.Mult)
...
@test (binop.left.col_offset == 3)
@test (binop.right.col_offset == 7)
binop = t.body[2].value.values[3].value
@test (type_(binop) == ast.BinOp)
@test (type_(binop.left) == ast.Name)
@test (type_(binop.op) == ast.Mult)
@test (type_(binop.right) == ast.Call)
...
@test (binop.left.lineno == 3)
@test (binop.right.lineno == 3)
@test (binop.col_offset == 13)
@test (binop.left.col_offset == 13)
@test (binop.right.col_offset == 17)
binop = t.body[2].value.values[5].value
@test (type_(binop) == ast.BinOp)
@test (type_(binop.left) == ast.Name)
@test (type_(binop.op) == ast.Mult)
@test (type_(binop.right) == ast.Call)
...
@test (binop.left.lineno == 3)
@test (binop.right.lineno == 3)
@test (binop.col_offset == 23)
@test (binop.left.col_offset == 23)
@test (binop.right.col_offset == 27)
end

function test_ast_numbers_fstring_with_formatting(self::@like(TestCase))
t = ast.parse("f\"Here is that pesky {xxx:.3f} again\"")
@test (length(t.body) == 1)
@test (t.body[1].lineno == 1)
@test (type_(t.body[1]) == ast.Expr)
@test (type_(t.body[1].value) == ast.JoinedStr)
@test (length(t.body[1].value.values) == 3)
@test (type_(t.body[1].value.values[1]) == ast.Constant)
@test (type_(t.body[1].value.values[2]) == ast.FormattedValue)
@test (type_(t.body[1].value.values[3]) == ast.Constant)
(_, expr, _) = t.body[1].value.values
name = expr.value
@test (type_(name) == ast.Name)
@test (name.lineno == 1)
@test (name.end_lineno == 1)
@test (name.col_offset == 22)
@test (name.end_col_offset == 25)
end

function test_ast_line_numbers_multiline_fstring(self::@like(TestCase))
...
@test (type_(t.body[2]) == ast.Expr)
@test (type_(t.body[2].value) == ast.JoinedStr)
@test (length(t.body[2].value.values) == 3)
@test (type_(t.body[2].value.values[1]) == ast.Constant)
@test (type_(t.body[2].value.values[1].value) == String)
@test (type_(t.body[2].value.values[2]) == ast.FormattedValue)
@test (type_(t.body[2].value.values[3]) == ast.Constant)
...
@test (t.body[2].value.values[2].lineno == 3)
@test (t.body[2].value.values[3].lineno == 3)
@test (t.body[2].col_offset == 0)
@test (t.body[2].value.col_offset == 0)
@test (t.body[2].value.values[1].col_offset == 0)
@test (t.body[2].value.values[2].col_offset == 0)
@test (t.body[2].value.values[3].col_offset == 0)
binop = t.body[2].value.values[2].value
@test (type_(binop) == ast.BinOp)
@test (type_(binop.left) == ast.Name)
@test (type_(binop.op) == ast.Mult)
@test (type_(binop.right) == ast.Call)
@test (binop.lineno == 4)
@test (binop.left.lineno == 4)
@test (binop.right.lineno == 6)
@test (binop.col_offset == 4)
@test (binop.left.col_offset == 4)
@test (binop.right.col_offset == 7)
end

function test_ast_line_numbers_with_parentheses(self::@like(TestCase))
expr = "\nx = (\n    f\" {test(t)}\"\n)"
t = ast.parse(expr)
@test (type_(t) == ast.Module)
@test (length(t.body) == 1)
call = t.body[1].value.values[2].value
@test (type_(call) == ast.Call)
@test (call.lineno == 3)
@test (call.end_lineno == 3)
@test (call.col_offset == 8)
@test (call.end_col_offset == 15)
expr = "\nx = (\n        \'PERL_MM_OPT\', (\n            f\'wat\'\n            f\'some_string={f(x)} \'\n            f\'wat\'\n        ),\n)\n"
t = ast.parse(expr)
@test (type_(t) == ast.Module)
@test (length(t.body) == 1)
fstring = t.body[1].value.elts[2]
@test (type_(fstring) == ast.JoinedStr)
@test (length(fstring.values) == 3)
(wat1, middle, wat2) = fstring.values
@test (type_(wat1) == ast.Constant)
@test (wat1.lineno == 4)
@test (wat1.end_lineno == 6)
@test (wat1.col_offset == 12)
@test (wat1.end_col_offset == 18)
call = middle.value
@test (type_(call) == ast.Call)
@test (call.lineno == 5)
@test (call.end_lineno == 5)
@test (call.col_offset == 27)
@test (call.end_col_offset == 31)
@test (type_(wat2) == ast.Constant)
@test (wat2.lineno == 4)
@test (wat2.end_lineno == 6)
@test (wat2.col_offset == 12)
@test (wat2.end_col_offset == 18)
end

function test_docstring(self::@like(TestCase))
function f()
"Not a docstring"
end

assertIsNone(self, f.__doc__)
function g()
"Not a docstring"
end

assertIsNone(self, g.__doc__)
end

function test_literal_eval(self::@like(TestCase))
assertRaisesRegex(self, ValueError, "malformed node or string") do 
ast.literal_eval("f\'x\'")
end
end

function test_ast_compile_time_concat(self::@like(TestCase))
x = [""]
expr = "x[0] = \'foo\' f\'{3}\'"
t = ast.parse(expr)
c = compile(t, "", "exec")
...
function test_compile_time_concat_errors(self::@like(TestCase))
assertAllRaise(self, SyntaxError, "cannot mix bytes and nonbytes literals", ["f\'\' b\'\'", "b\'\' f\'\'"])
end

function test_literal(self::@like(TestCase))
@test ("" == "")
@test ("a" == "a")
@test (" " == " ")
end

function test_unterminated_string(self::@like(TestCase))
assertAllRaise(self, SyntaxError, "f-string: unterminated string", ["f\'{\"x\'", "f\'{\"x}\'", "f\'{(\"x\'", "f\'{(\"x}\'"])
end

function test_mismatched_parens(self::@like(TestCase))
assertAllRaise(self, SyntaxError, "f-string: closing parenthesis \'\\}\' does not match opening parenthesis \'\\(\'", ["f\'{((}\'"])
assertAllRaise(self, SyntaxError, "f-string: closing parenthesis \'\\)\' does not match opening parenthesis \'\\[\'", ["f\'{a[4)}\'"])
assertAllRaise(self, SyntaxError, "f-string: closing parenthesis \'\\]\' does not match opening parenthesis \'\\(\'", ["f\'{a(4]}\'"])
assertAllRaise(self, SyntaxError, "f-string: closing parenthesis \'\\}\' does not match opening parenthesis \'\\[\'", ["f\'{a[4}\'"])
assertAllRaise(self, SyntaxError, "f-string: closing parenthesis \'\\}\' does not match opening parenthesis \'\\(\'", ["f\'{a(4}\'"])
@test_throws
...
function test_double_braces(self::@like(TestCase))
@test ("{" == "{")
@test ("a{" == "a{")
@test ("{b" == "{b")
@test ("a{b" == "a{b")
...
@test ("$(10)}a{}" == "10}a{}")
@test ("$("{{}}")" == "{{}}")
assertAllRaise(self, TypeError, "unhashable type", ["f\'{ {{}} }\'"])
end

function test_compile_time_concat(self::@like(TestCase))
x = "def"
@test ("abc## $(x)ghi" == "abc## defghi")
@test ("abc$(x)ghi" == "abcdefghi")
@test ("abc$(x)ghi$(round(x, digits="4"))" == "abcdefghidef ")
@test ("{x}$(x)" == "{x}def")
@test ("{x$(x)" == "{xdef")
@test ("{x}$(x)" == "{x}def")
@test ("{{x}}$(x)" == "{{x}}def")
...
@test ("" == "")
@test ("" == "")
assertAllRaise(self, SyntaxError, "f-string: expecting \'}\'", ["f\'{3\' f\'}\'"])
end

function test_comments(self::@like(TestCase))
d = Dict{String, String}("#" => "hash")
@test ("$("#")" == "#")
@test ("$(d["#"])" == "hash")
assertAllRaise(self, SyntaxError, "f-string expression part cannot include \'#\'", ["f\'{1#}\'", "f\'{3(#)}\'", "f\'{#}\'"])
assertAllRaise(self, SyntaxError, "f-string: unmatched \'\\)\'", ["f\'{)#}\'"])
end

function test_many_expressions(self::@like(TestCase))
function build_fstr(n::@like(TestCase), extra = "")
return (("f\'" + "{x} "*n) + extra) + "\'"
end
...
width = 1
for i in 250:259
@test (py"build_fstr(i)" == repeat(x * " ",i))
end
@test (py"build_fstr(255)*256" == repeat(x * " ",255*256))
s = build_fstr(253, "{x:{width}} ")
@test (py"s" == repeat(x * " ",254))
s = repeat("f\'{1}\' \'x\' \'y\'",1024)
@test (py"s" == repeat("1xy",1024))
end

function test_format_specifier_expressions(self::@like(TestCase))
width = 10
precision_ = 4
value = decimal.Decimal("12.34567")
@test ("result: $(round(value, digits=$(precision_)))" == "result:      12.35")
@test ("result: $(round(value, digits=$(precision_)))" == "result:      12.35")
@test ("result: $(round(value, digits=$(round(precision_, digits=1))))" == "result:      12.35")
@test ("result: $(round(value, digits=$(round(precision_, digits=1))))" == "result:      12.35")
@test ("result: $(round(value, digits=$(round(precision_, digits=1))))" == "result:      12.35")
@test ("$(round(10, digits="#$(1)0x"))" == "       0xa")
@test ("$(round(10, digits="$("#")1$(0)$("x")"))" == "       0xa")
@test ("$(round(-10, digits="-$("#")1$(0)x"))" == "      -0xa")
@test ("$(round(-10, digits="$("-")#$(1)0$("x")"))" == "      -0xa")
@test ("$(round(10, digits="#$(3 != Dict{int, int}(4 => 5)&&width)x"))" == "       0xa")
assertAllRaise(self, SyntaxError, "f-string: expecting \'}\'", ["f\'{\"s\"!r{\":10\"}}\'"])
assertAllRaise(self, SyntaxError, "f-string: invalid syntax", ["f\'{4:{/5}}\'"])
assertAllRaise(self, SyntaxError, "f-string: expressions nested too deeply", ["f\'result: {value:{width:{0}}.{precision:1}}\'"])
assertAllRaise(self, SyntaxError, "f-string: invalid conversion character", ["f\'{\"s\"!{\"r\"}}\'"])
end

function test_side_effect_order(self::@like(TestCase))
x = X()
@test ("$(x) $(x)" == "1 2")
end

function test_missing_expression(self::@like(TestCase))
assertAllRaise(self, SyntaxError, "f-string: empty expression not allowed", ["f\'{}\'", "f\'{ }\'f\' {} \'", "f\'{!r}\'", "f\'{ !r}\'", "f\'{10:{ }}\'", "f\' { } \'", "f\'\'\'{\t\f\r\n}\'\'\'", "f\'{!x}\'", "f\'{ !xr}\'", "f\'{!x:}\'", "f\'{!x:a}\'", "f\'{ !xr:}\'", "f\'{ !xr:a}\'", "f\'{!}\'", "f\'{:}\'", "f\'{!\'", "f\'{!s:\'", "f\'{:\'", "f\'{:x\'"])
assertAllRaise(self, SyntaxError, "invalid non-printable character U\\+00A0", ["f\'\'\'{ }\'\'\'", " "])
end

function test_parens_in_expressions(self::@like(TestCase))
@test ("$((3,))" == "(3,)")
assertAllRaise(self, SyntaxError, "f-string: invalid syntax", ["f\'{,}\'", "f\'{,}\'"])
assertAllRaise(self, SyntaxError, "f-string: unmatched \'\\)\'", ["f\'{3)+(4}\'"])
assertAllRaise(self, SyntaxError, "unterminated string literal", ["f\'{\n}\'"])
end

function test_newlines_before_syntax_error(self::@like(TestCase))
assertAllRaise(self, SyntaxError, "invalid syntax", ["f\'{.}\'", "\nf\'{.}\'", "\n\nf\'{.}\'"])
end

function test_backslashes_in_string_part(self::@like(TestCase))
@test ("\t" == "\t")
@test ("\\t" == "\\t")
@test ("\\t" == "\\t")
@test ("$(2)\t" == "2\t")
...
AMPERSAND = "spam"
@test ("&" == "&")
@test ("\\N$(AMPERSAND)" == "\\Nspam")
@test ("\\N$(AMPERSAND)" == "\\Nspam")
@test ("\\&" == "\\&")
end

function test_misformed_unicode_character_name(self::@like(TestCase))
assertAllRaise(self, SyntaxError, "\\(unicode error\\) \'unicodeescape\' codec can\'t decode bytes in position .*: malformed \\\\N character escape", ["f\'\\N\'", "f\'\\N \'", "f\'\\N  \'", "f\'\\N{\'", "f\'\\N{GREEK CAPITAL LETTER DELTA\'", "\'\\N\'", "\'\\N \'", "\'\\N  \'", "\'\\N{\'", "\'\\N{GREEK CAPITAL LETTER DELTA\'"])
end

function test_no_backslashes_in_expression_part(self::@like(TestCase))
assertAllRaise(self, SyntaxError, "f-string expression part cannot include a backslash", ["f\'{\\\'a\\\'}\'", "f\'{\\t3}\'", "f\'{\\}\'", "rf\'{\\\'a\\\'}\'", "rf\'{\\t3}\'", "rf\'{\\}\'", "rf\'{\"\\N{LEFT CURLY BRACKET}\"}\'", "f\'{\\n}\'"])
end

function test_no_escapes_for_braces(self::@like(TestCase))
#= 
...
function test_newlines_in_expressions(self::@like(TestCase))
@test ("$(0)" == "0")
@test ("$(3 + 4)" == "7")
end

function test_lambda(self::@like(TestCase))
x = 5
@test ("$((y) -> x*y("8"))" == "\'88888\'")
@test ("$(round('(y) -> x*y("8")', digits="10"))" == "\'88888\'   ")
@test ("$(round((y) -> x*y("8"), digits="10"))" == "88888     ")
assertAllRaise(self, SyntaxError, "f-string: invalid syntax", ["f\'{lambda x:x}\'"])
end

@resumable function test_yield(self::@like(TestCase))
g = fn(4)
@test (next(g) == 8)
@test (next(g) == nothing)
end

@resumable function test_yield_send(self::@like(TestCase))
g = fn(10)
the_lambda = next(g)
@test (the_lambda(4) == 40)
@test (send(g, "string") == "x:string")
end

function test_expressions_with_triple_quoted_strings(self::@like(TestCase))
@test ("$("x")" == "x")
@test ("$("eric\'s")" == "eric\'s")
@test ("$("xeric\"sy")" == "xeric\"sy")
@test ("$("xeric\"s")" == "xeric\"s")
@test ("$("eric\"sy")" == "eric\"sy")
@test ("$("xeric\"sy")" == "xeric\"sy")
@test ("$("xeric\"sy")" == "xeric\"sy")
@test ("$("xeric\"sy")" == "xeric\"sy")
end

function test_multiple_vars(self::@like(TestCase))
x = 98
y = "abc"
@test ("$(x)$(y)" == "98abc")
@test ("X$(x)$(y)" == "X98abc")
@test ("$(x)X$(y)" == "98Xabc")
@test ("$(x)$(y)X" == "98abcX")
@test ("X$(x)Y$(y)" == "X98Yabc")
@test ("X$(x)$(y)Y" == "X98abcY")
@test ("$(x)X$(y)Y" == "98XabcY")
@test ("X$(x)Y$(y)Z" == "X98YabcZ")
end

function test_closure(self::@like(TestCase))
function outer(x::@like(TestCase))
function inner()
return "x:$(x)"
end
...
@test (outer("987")() == "x:987")
@test (outer(7)() == "x:7")
end

function test_arguments(self::@like(TestCase))
y = 2
function f(x::@like(TestCase), width)
return "x=$(round(x*y, digits="$(width)"))"
end

@test (f("foo", 10) == "x=foofoo    ")
x = "bar"
@test (f(10, 10) == "x=        20")
end

function test_locals(self::@like(TestCase))
value = 123
@test ("v:$(value)" == "v:123")
end

function test_missing_variable(self::@like(TestCase))
@test_throws NameError do 
"v:$(value)"
end
end

function test_missing_format_spec(self::@like(TestCase))
@test ("$(round(O(), digits="x"))" == "x")
@test ("$(O())" == "*")
@test ("$(round(O(), digits=""))" == "*")
@test ("$(round(3, digits=""))" == "3")
@test ("$(round(3, digits=""))" == "3")
end

function test_global(self::@like(TestCase))
@test ("g:$(a_global)" == "g:global variable")
@test ("g:$(a_global)" == "g:\'global variable\'")
a_local = "local variable"
@test ("g:$(a_global) l:$(a_local)" == "g:global variable l:local variable")
@test ("g:$(a_global)" == "g:\'global variable\'")
@test ("g:$(a_global) l:$(a_local)" == "g:global variable l:\'local variable\'")
assertIn(self, "module \'unittest\' from", "$(unittest)")
end

function test_shadowed_global(self::@like(TestCase))
a_global = "really a local"
@test ("g:$(a_global)" == "g:really a local")
@test ("g:$(a_global)" == "g:\'really a local\'")
a_local = "local variable"
@test ("g:$(a_global) l:$(a_local)" == "g:really a local l:local variable")
@test ("g:$(a_global)" == "g:\'really a local\'")
@test ("g:$(a_global) l:$(a_local)" == "g:really a local l:\'local variable\'")
end

function test_call(self::@like(TestCase))
function foo(x::@like(TestCase))::String
return "x=" * string(x)
end
...
function test_nested_fstrings(self::@like(TestCase))
y = 5
@test ("$("$(0)"*3)" == "000")
@test ("$("$(y)"*3)" == "555")
end

function test_invalid_string_prefixes(self::@like(TestCase))
single_quote_cases = ["fu\'\'", "uf\'\'", "Fu\'\'", "fU\'\'", "Uf\'\'", "uF\'\'", "ufr\'\'", "urf\'\'", "fur\'\'", "fru\'\'", "rfu\'\'", "ruf\'\'", "FUR\'\'", "Fur\'\'", "fb\'\'", "fB\'\'", "Fb\'\'", "FB\'\'", "bf\'\'", "bF\'\'", "Bf\'\'", "BF\'\'"]
double_quote_cases = [replace(case, "\'", "\"") for case in single_quote_cases]
assertAllRaise(self, SyntaxError, "invalid syntax", single_quote_cases + double_quote_cases)
end

function test_leading_trailing_spaces(self::@like(TestCase))
@test ("$(3)" == "3")
@test ("$(3)" == "3")
@test ("$(3)" == "3")
@test ("$(3)" == "3")
@test ("expr=$(Dict(x => y for (x, y) in [(1, 2)]))" == "expr={1: 2}")
@test ("expr=$(Dict(x => y for (x, y) in [(1, 2)]))" == "expr={1: 2}")
end

function test_not_equal(self::@like(TestCase))
@test ("$(3 != 4)" == "True")
@test ("$(round(3 != 4, digits=""))" == "True")
@test ("$(3 != 4)" == "True")
@test ("$(round(3 != 4, digits=3))" == "Tru")
end

function test_equal_equal(self::@like(TestCase))
@test ("$(0 == 1)" == "False")
end

function test_conversions(self::@like(TestCase))
@test ("$(round(3.14, digits=10))" == "      3.14")
@test ("$(round(3.14, digits=10))" == "3.14      ")
@test ("$(round('3.14', digits=10))" == "3.14      ")
@test ("$(round(3.14, digits=10))" == "3.14      ")
@test ("$("a")" == "a")
...
@test ("$("a!r")" == "a!r")
@test ("$(round(3.14, digits=10))" == "3.14!!!!!!")
assertAllRaise(self, SyntaxError, "f-string: invalid conversion character", ["f\'{3!g}\'", "f\'{3!A}\'", "f\'{3!3}\'", "f\'{3!G}\'", "f\'{3!!}\'", "f\'{3!:}\'", "f\'{3! s}\'"])
assertAllRaise(self, SyntaxError, "f-string: expecting \'}\'", ["f\'{x!s{y}}\'", "f\'{3!ss}\'", "f\'{3!ss:}\'", "f\'{3!ss:s}\'"])
end

function test_assignment(self::@like(TestCase))
assertAllRaise(self, SyntaxError, "invalid syntax", ["f\'\' = 3", "f\'{0}\' = x", "f\'{x}\' = x"])
end

function test_del(self::@like(TestCase))
assertAllRaise(self, SyntaxError, "invalid syntax", ["del f\'\'", "del \'\' f\'\'"])
end

function test_mismatched_braces(self::@like(TestCase))
assertAllRaise(self, SyntaxError, "f-string: single \'}\' is not allowed", ["f\'{{}\'", "f\'{{}}}\'", "f\'}\'", "f\'x}\'", "f\'x}x\'", "f\'\\u007b}\'", "f\'{3:}>10}\'", "f\'{3:}}>10}\'"])
assertAllRaise(self, SyntaxError, "f-string: expecting \'}\'", ["f\'{3:{{>10}\'", "f\'{3\'", "f\'{3!\'", "f\'{3:\'", "f\'{3!s\'", "f\'{3!s:\'", "f\'{3!s:3\'", "f\'x{\'", "f\'x{x\'", "f\'{x\'", "f\'{3:s\'", "f\'{{{\'", "f\'{{}}{\'", "f\'{\'", "f\'x{<\'", "f\'x{>\'"])
@test ("$("{")" == "{")
@test ("$("}")" == "}")
@test ("$(round(3, digits="$("}")>10"))" == "}}}}}}}}}3")
@test ("$(round(2, digits="$("{")>10"))" == "{{{{{{{{{2")
end

function test_if_conditional(self::@like(TestCase))
function test_fstring(x::@like(TestCase), expected)
flag = 0
if "$(x)"
flag = 1
else
...
function test_concat_empty(x::@like(TestCase), expected)
flag = 0
if "$(x)"
flag = 1
else
...
function test_concat_non_empty(x::@like(TestCase), expected)
flag = 0
if " $(x)"
flag = 1
else
...
function test_empty_format_specifier(self::@like(TestCase))
x = "test"
@test ("$(x)" == "test")
@test ("$(round(x, digits=""))" == "test")
@test ("$(round(x, digits=""))" == "test")
@test ("$(round('x', digits=""))" == "\'test\'")
end

function test_str_format_differences(self::@like(TestCase))
d = Dict{Any, String}("a" => "string", 0 => "integer")
a = 0
@test ("$(d[0])" == "integer")
@test ("$(d["a"])" == "string")
@test ("$(d[a])" == "integer")
...
function test_errors(self::@like(TestCase))
assertAllRaise(self, TypeError, "unsupported", ["f\'{(lambda: 0):x}\'", "f\'{(0,):x}\'"])
assertAllRaise(self, ValueError, "Unknown format code", ["f\'{1000:j}\'", "f\'{1000:j}\'"])
end

function test_filename_in_syntaxerror(self::@like(TestCase))
temp_cwd() do cwd 
file_path = os.join(cwd, "t.py")
readline(file_path) do f 
write(f, "f\"{a b}\"")
end
(_, _, stderr) = assert_python_failure(file_path, PYTHONIOENCODING = "ascii")
end
assertIn(self, encode(file_path, "ascii", "backslashreplace"), stderr)
end

function test_loop(self::@like(TestCase))
for i in 0:999
@test ("i:$(i)" == "i:" * string(i))
end
end

function test_dict(self::@like(TestCase))
d = Dict{String, String}("\"" => "dquote", "\'" => "squote", "foo" => "bar")
@test ("$(d["\'"])" == "squote")
@test ("$(d["\""])" == "dquote")
@test ("$(d["foo"])" == "bar")
@test ("$(d["foo"])" == "bar")
...
function test_backslash_char(self::@like(TestCase))
@test (py""f\"\\\n\""" == "")
@test (py""f\"\\\r\""" == "")
end

function test_debug_conversion(self::@like(TestCase))
x = "A string"
@test ("x=$(x)" == "x=" + repr(x))
@test ("x =$(x)" == "x =" + repr(x))
@test ("x=$(x)" == "x=" * string(x))
...
@test ("x=$(x)" == "x=" + ascii(x))
x = 2.71828
@test ("x=$(round(x, digits=2f))" == "x=" + x)
@test ("x=$(round(x, digits=""))" == "x=" + x)
@test ("x=$(round('x', digits="^20"))" == "x=" + repr(x))
@test ("x=$(round(x, digits="^20"))" == "x=" + "^20")
@test ("x=$(round(x, digits="^20"))" == "x=" + ascii(x))
x = 9
@test ("3*x+15=$(repeat(x,3) + 15)" == "3*x+15=42")
tenπ = 31.4
@test ("tenπ=$(round(tenπ, digits=2f))" == "tenπ=31.40")
@test ("\"Σ\"=$("Σ")" == "\"Σ\"=\'Σ\'")
@test ("$(round("3.1415=$(round(3.1415, digits=1f))", digits="*^20"))" == "*****3.1415=3.1*****")
pi = "π"
@test ("alpha α pi=$(pi) ω omega" == "alpha α pi=\'π\' ω omega")
@test ("\n3\n=$(3)" == "\n3\n=3")
@test ("$(0 == 1)" == "False")
@test ("$(0 != 1)" == "True")
@test ("$(0 <= 1)" == "True")
...
@test ("$("=")" == "=")
x = 20
@test ("$(round(x, digits="=10"))" == "        20")
function f(a::@like(TestCase))
# Not Supported
# nonlocal x
oldx = x
x = a
...
@test ("C()=$(C())" == "C()=REPR")
@test ("C()=$(C())" == "C()=REPR")
@test ("C()=$(round(C(), digits=""))" == "C()=FORMAT-")
@test ("C()=$(round(C(), digits=" "))" == "C()=FORMAT- ")
@test ("C()=$(round(C(), digits="x"))" == "C()=FORMAT-x")
@test ("C()=$(round('C()', digits="*^20"))" == "C()=********REPR********")
@test_throws
x = "foo"
@test ("Xx=$(x)Y" == ("Xx=" + repr(x)) + "Y")
@test ("Xx  =$(x)Y" == ("Xx  =" + repr(x)) + "Y")
@test ("Xx=  $(x)Y" == ("Xx=  " + repr(x)) + "Y")
@test ("Xx  =  $(x)Y" == ("Xx  =  " + repr(x)) + "Y")
end

function test_walrus(self::@like(TestCase))
x = 20
@test ("$(round(x, digits="=10"))" == "        20")
@test ("$((x = 10))" == "10")
@test (x == 10)
...
function test_invalid_syntax_error_message(self::@like(TestCase))
assertRaisesRegex(self, SyntaxError, "f-string: invalid syntax") do 
compile("f\'{a \$ b}\'", "?", "exec")
end
end

function test_with_two_commas_in_format_specifier(self::@like(TestCase))
...
function test_with_two_underscore_in_format_specifier(self::@like(TestCase))
...
function test_with_a_commas_and_an_underscore_in_format_specifier(self::@like(TestCase))
...
function test_with_an_underscore_and_a_comma_in_format_specifier(self::@like(TestCase))
error_msg = re.escape("Cannot specify both \',\' and \'_\'.")
assertRaisesRegex(self, ValueError, error_msg) do 
"$(round(1, digits="_,"))"
end
...
function test_syntax_error_for_starred_expressions(self::@like(TestCase))
error_msg = re.escape("cannot use starred expression here")
assertRaisesRegex(self, SyntaxError, error_msg) do 
compile("f\'{*a}\'", "?", "exec")
end
